package one.modality.base.backoffice.controls.multilangeditor;
...
public final class MultiLanguageEditor {
  ...
  private static final String[] languages = {"en", "de", "es", "fr", "pt"};
  private static final String entityListId = "loadedEntity";
  ...
  private final Map<Pair<Object /*entityId**/, Object /*language*/>, MonoLanguageEditor>
      monoLanguageEditors = new HashMap<>();

  public MultiLanguageEditor(
      ButtonFactoryMixin buttonFactory,
      Entity entity,
      Function<Object, Object> bodyFieldGetter,
      Function<Object, Object> subjectFieldGetter) {
    this(
        buttonFactory, entity::getId, entity.getStore(), bodyFieldGetter, subjectFieldGetter, null);
  ...
  public MultiLanguageEditor(
      ...
      Function<Object, Object> bodyFieldGetter,
      Function<Object, Object> subjectFieldGetter,
      String domainClassIdOrLoadingSelect) {
    this(
        buttonFactory,
        entityIdGetter,
        ...
        bodyFieldGetter,
        subjectFieldGetter,
        domainClassIdOrLoadingSelect);
  }

  public MultiLanguageEditor(
      ButtonFactoryMixin buttonFactory,
      Callable entityIdGetter,
      EntityStore loadingStore,
      Function<Object, Object> bodyFieldGetter,
      Function<Object, Object> subjectFieldGetter,
      String domainClassIdOrLoadingSelect) {
    ...
    StringBuilder sb =
        domainClassIdOrLoadingSelect == null || domainClassIdOrLoadingSelect.startsWith("select ")
            ? null
            : new StringBuilder("select ");
    for (String language : languages) {
      ToggleButton languageButton =
          new ToggleButton(null, ModalityIcons.getLanguageIcon32(language));
      languageButton.setUserData(language);
      languageButton.setMinWidth(50d);
      languageButtons.put(language, languageButton);
      if (sb != null) {
        if (!Objects.areEquals(language, languages[0])) sb.append(',');
        sb.append(bodyFieldGetter.apply(language));
        if (subjectFieldGetter != null) sb.append(',').append(subjectFieldGetter.apply(language));
      }
    }
    this.loadingSelect =
        sb == null
            ? domainClassIdOrLoadingSelect
            : sb.append(" from ")
                .append(domainClassIdOrLoadingSelect)
                .append(" where id=?")
                .toString();
  ...
  public BorderPane getUiNode() {
    if (toggleGroup.getSelectedToggle() == null) {
      FXProperties.runOnPropertiesChange(
          this::onEntityChanged, toggleGroup.selectedToggleProperty());
  ...
  public void onEntityChanged() {
    ...
    if (monoLanguageEditor == null) return;
    ...
    if (entityUpdates.containsKey(entityId))
      ...
      loadingStore
          .executeListQuery(entityListId, loadingSelect, entityId)
          .onSuccess(
              entities -> {
  ...
  private MonoLanguageEditor getCurrentMonoLanguageEditor() {
    ...
    Object language =
        selectedLanguageButton != null ? selectedLanguageButton.getUserData() : I18n.getLanguage();
  ...
  private final class MonoLanguageEditor {
    ...
    private final Button saveButton =
        buttonFactory.newButton(
            closeCallback != null ? OK_ACTION_KEY : SAVE_ACTION_KEY, this::save);
    private final Button revertButton =
        buttonFactory.newButton(
            closeCallback != null ? CANCEL_ACTION_KEY : REVERT_ACTION_KEY, this::revert);
    ...
    MonoLanguageEditor(Object lang) {
      ...
      FXProperties.runOnPropertiesChange(
          this::syncEntityFromUi, subjectTextField.textProperty(), editor.textProperty());
    }
    ...
    void syncUiFromEntity() {
      if (editedEntity != null) {
        ...
        if (subjectField != null) {
          String uiSubject = subjectTextField.getText();
          String entitySubject = updatedEntity.getStringFieldValue(subjectField);
          if (!Objects.areEquals(uiSubject, entitySubject)) subjectTextField.setText(entitySubject);
        }
        String uiBody = editor.getText();
        String entityBody = updatedEntity.getStringFieldValue(bodyField);
        if (!Objects.areEquals(uiBody, entityBody)) editor.setText(entityBody);
        updateButtonsDisable();
      }
    }
    ...
    void callCloseCallback(boolean saved) {
      if (closeCallback != null) closeCallback.handle(saved ? editedEntity.loadedEntity : null);
    }
    ...
    void displayEditor() {
      if (borderPane.getCenter() != editor) {
        syncUiFromEntity();
        if (subjectField != null) borderPane.setTop(subjectTextField);
        borderPane.setCenter(editor);
        BorderPane buttonsBar = new BorderPane();
        HBox hBox = new HBox();
        for (Object language : languages) hBox.getChildren().add(languageButtons.get(language));
        buttonsBar.setLeft(hBox);
        buttonsBar.setCenter(
            new HBox(
                20,
        ...
        // The following code is just a temporary workaround to make CKEditor work in html platform
