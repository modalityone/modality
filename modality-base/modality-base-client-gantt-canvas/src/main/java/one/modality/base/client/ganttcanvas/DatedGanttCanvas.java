...
public final class DatedGanttCanvas implements TimeWindow<LocalDate> {
  ...
  public DatedGanttCanvas() {
    ...
    // Note: the order is important regarding the vertical strips, so they are always displayed "in
    // the background".
    // For example, if days are used to draw the strips, they must be drawn before drawing weeks,
    // months, etc...
    // otherwise these strips would appear on top of them instead of behind.

    // Some layout properties (such as layer visibilities, vertical position, height...) must be set
    // before the layout pass
    globalLayout.addOnBeforeLayout(this::setLayoutPropertiesBeforeLayoutPass);
    // Some draw properties (such as fonts, colors, paddings, radius...) must be set before the draw
    // pass
  ...
  private void setDrawPropertiesBeforeDrawPass() { // Called only once before the draw pass
    ...
    // 2) Properties that depend on the year selection => drawYear() will set the correct value in
    // yearBarDrawer on each call
    ...
    weekBarDrawer
        ...
        .setBottomTextFont(
            TextTheme.getFont(FontDef.font(FontWeight.BOLD, weekFontSize))) // bottom = week number
    ...
    // 2) Properties that depend on the week selection => drawWeek() will set the correct value in
    // weekBarDrawer on each call
    ...
    dayBarDrawer
        .setTopTextFont(
            TextTheme.getFont(
                FontDef.font(0.6 * dayOfMonthFontSize))) // top = day of week => smaller
        .setBottomTextFont(
            TextTheme.getFont(
                FontDef.font(FontWeight.BOLD, dayOfMonthFontSize))) // bottom = day of month
  ...
  private static final Interpolator FONT_SIZE_INTERPOLATOR = Interpolator.SPLINE(0.5, 0.5, 0.1, 1);
