...
public final class ClientSessionRecorderJob implements ApplicationJob {
  ...
  private void loadSessionApplication() {
    sessionApplication =
        recreateSessionEntityFromLocalStorage("SessionApplication", "sessionApplication.id");
  }
  ...
  private void onShutdown() {
    ...
    if (sessionProcess != null) {
      ...
      if (sessionConnection != null) touchSessionEntityEnd(store.updateEntity(sessionConnection));
  ...
  private void stopListeningServerPushCalls() {
    // Resetting the push client id property to null (will be reassigned when connected again). The
    // purpose is to
  ...
  private Entity recreateSessionEntityFromLocalStorage(Object domainClassId, String idKey) {
    return store.createEntity(
        EntityId.create(
            store.getDomainClass(domainClassId), Integer.parseInt(LocalStorage.getItem(idKey))));
  }
