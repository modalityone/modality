...
final class FiltersActivity extends EventDependentViewDomainActivity
  ...
  public Node buildUi() {
    ...
    ButtonSelector<DomainClass> classSelector =
        new ButtonSelector<>(this, container) {
          ...
          private final VisualGrid dialogVisualGrid =
              new SkinnedVisualGrid(); // Better rendering in desktop JavaFX (but might be slower in
                                       // web version)

          {
            ...
            dialogVisualGrid
                ...
                .addListener(
                    (observable, oldValue, vs) -> {
                      setSelectedItem(
                          vs == null || vs.getSelectedRow() == 0
                              ? null
                              : searchedClasses.get(vs.getSelectedRow() - 1));
            ...
            searchedClasses =
                allClasses.stream()
                    .filter(
                        c ->
                            classSearch == null
                                || c.getName().toLowerCase().contains(classSearch.toLowerCase()))
                    .collect(Collectors.toList());
            VisualResultBuilder vsb =
                VisualResultBuilder.create(
                    searchedClasses.size() + 1, VisualColumn.create(TextRenderer.SINGLETON));
    ...
    Button addNewFilterButton =
        newButton(newOperationAction(() -> new AddNewFilterRequest(getEventStore(), container)));
    HBox filterSearchRow =
        new HBox(classLabel, classSelector.getButton(), filterSearchField, addNewFilterButton);
    ...
    ObservableValue<Boolean> isSelectedFilterNull =
        FXProperties.compute(selectedFilter, Objects::isNull); // WebFX replacement
    fieldSearchField.disableProperty().bind(isSelectedFilterNull);
    Button addNewFieldsButton =
        newButton(newOperationAction(() -> new AddNewFieldsRequest(getEventStore(), container)));
    ...
    filterAndFieldPaneBorder.setBorder(
        new Border(new BorderStroke(Color.LIGHTGRAY, BorderStrokeStyle.SOLID, null, null)));
  ...
  */
  ...
  protected void startLogic() {
    ...
    filtersVisualMapper =
        ReactiveVisualMapper.<Filter>createPushReactiveChain(this)
            .always(
            ...
            .ifTrimNotEmpty(
                pm.filterClassProperty(), s -> where("lower(class) = ?", s.toLowerCase()))
            ...
            .addEntitiesHandler(
                entities -> {
                  // If the filter table is cleared (e.g. by the user entering text in the search
                  // field) set the selected filter property to null
                  if (entities.isEmpty()) {
        ...
        ReactiveVisualMapper.<Filter>createPushReactiveChain(this)
            ...
            .setEntityColumns(
                "["
                    ...
                    "]")
            .ifTrimNotEmpty(
                pm.fieldsSearchTextProperty(),
                s -> where("lower(name) like ?", "%" + s.toLowerCase() + "%"))
            .always(
                selectedFilter,
                s ->
                    s != null
                        ? where("lower(class) = ?", s.getClassId().toString().toLowerCase())
                        : where("1 = 0"))
            .applyDomainModelRowStyle() // Colorizing the rows
            .autoSelectSingleRow() // When the result is a singe row, automatically select it
            ...
            .addEntitiesHandler(
                entities -> {
                  // If the fields' table is cleared (e.g. by the user entering text in the search
                  // field) set the selected fields property to null
                  if (entities.isEmpty()) {
  ...
  private void populateFilterTable() {
    ...
    String status =
        "Loading " + selectedFields.get().getName() + " columns for " + selectedClass + ".";
    displayStatus(status);

    filterFieldsResultVisualMapper =
        ReactiveVisualMapper.<Entity>createPushReactiveChain(this)
            .always(
                "{class: '"
                    + selectedClass
                    + "', alias: 'ma', columns: '"
            ...
            .ifNotNull(
                selectedFilter,
                s ->
                    s.getOrderByClause() != null && !s.getOrderByClause().isEmpty()
                        ? parse("orderBy: '" + s.getOrderByClause() + "'")
                        : null)
            .ifNotNull(
                pm.organizationIdProperty(), organization -> where("organization=?", organization))
            // .ifTrimNotEmpty(pm.searchTextProperty(), s -> where("name like ?",
