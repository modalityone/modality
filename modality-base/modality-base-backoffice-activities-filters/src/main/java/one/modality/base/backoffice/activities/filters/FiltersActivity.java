...
final class FiltersActivity extends EventDependentViewDomainActivity
    ...
    public Node buildUi() {
                ...
                new ButtonSelector<>(this, container) {
...

                    {
                        ...
                        FXProperties.runOnPropertiesChange(
                                this::updateDialogVisualGrid, searchTextProperty());
                    }
                    ...
                    private void updateDialogVisualGrid() {
                        ...
                        VisualResultBuilder vsb =
                                VisualResultBuilder.create(
                                        searchedClasses.size() + 1,
                                        VisualColumn.create(TextRenderer.SINGLETON));
        ...
        Button addNewFilterButton =
                newButton(
                        newOperationAction(
                                () -> new AddNewFilterRequest(getEventStore(), container)));
        HBox filterSearchRow =
                new HBox(
                        classLabel,
                        classSelector.getButton(),
                        filterSearchField,
                        addNewFilterButton);
        ...
        // BooleanBinding isSelectedFilterNull = selectedFilter.isNull(); // Not yet emulated by
        // WebFX
        ...
        Button addNewFieldsButton =
                newButton(
                        newOperationAction(
                                () -> new AddNewFieldsRequest(getEventStore(), container)));
    ...
    protected void startLogic() {
        ...
        filtersVisualMapper =
                ReactiveVisualMapper.<Filter>createPushReactiveChain(this)
                        ...
                        .addEntitiesHandler(
                                entities -> {
                                    // If the filter table is cleared (e.g. by the user entering
                                    // text in the search field) set the selected filter property to
                                    // null
                                    if (entities.isEmpty()) {
        ...
        fieldsVisualMapper =
                ReactiveVisualMapper.<Filter>createPushReactiveChain(this)
                        ...
                        .always(
                                ...
                                s ->
                                        s != null
                                                ? where(
                                                        "lower(class) = ?",
                                                        s.getClassId().toString().toLowerCase())
                                                : where("1 = 0"))
                        .applyDomainModelRowStyle() // Colorizing the rows
                        .autoSelectSingleRow() // When the result is a singe row, automatically
                                               // select it
                        ...
                        .addEntitiesHandler(
                                entities -> {
                                    // If the fields' table is cleared (e.g. by the user entering
                                    // text in the search field) set the selected fields property to
                                    // null
                                    if (entities.isEmpty()) {
        ...
        filterFieldsResultVisualMapper =
                ReactiveVisualMapper.<Entity>createPushReactiveChain(this)
                        ...
                        .ifNotNull(
                                selectedFilter,
                                s ->
                                        s.getOrderByClause() != null
                                                        && !s.getOrderByClause().isEmpty()
