...
final class ExportHelper {
      ...
      for (Expression<T> exp : ((ExpressionArray<T>) textExpression).getExpressions())
        concatExpression =
            concatExpression == null
                ? exp
                : new Plus<>(concatExpression, new Plus<>(Constant.newConstant(separator), exp));
  ...
  static <T> Expression<T> getTextExpression(
    ...
    if (expression instanceof Symbol) {
      ...
      if (subExpression != null) return getTextExpression(subExpression, excludeNumbers, skipArray);
    }
    ...
    if (topRightExpression instanceof DomainField) {
      ...
      if (foreignClass != null) {
        Expression<T> fte =
            getTextExpression(foreignClass.getForeignFields(), excludeNumbers, skipArray);
  ...
  static String getTextExpressionValue(Object value) {
    ...
    return value.toString() == null /*ex: NoValue*/
        ...
        : value
            ...
            .replace(
                '\n',
                ' '); // removing \n in order to paste the whole content in the same Excel cell (ex:
                      // special needs)
  }
}
