...
public final class MasterTableView implements UiBuilder {
    ...
    private MasterTableView(HasMasterVisualResultProperty pm, ControlFactoryMixin mixin) {
        ...
        if (pm instanceof HasMasterVisualSelectionProperty)
            ((HasMasterVisualSelectionProperty) pm)
                    .masterVisualSelectionProperty()
                    .bindBidirectional(masterTable.visualSelectionProperty());
        if (pm instanceof HasLimitProperty) {
            ...
            FXProperties.runNowAndOnPropertiesChange(
                    () ->
                            ((HasLimitProperty) pm)
                                    .limitProperty()
                                    .setValue(masterLimitCheckBox.isSelected() ? 30 : -1),
                    masterLimitCheckBox.selectedProperty());
    ...
    @Override
    public Node buildUi() {
        if (masterLimitCheckBox == null) return masterTable;
        BorderPane.setAlignment(masterTable, Pos.TOP_CENTER);
        return new BorderPane(masterTable, null, null, masterLimitCheckBox, null);
    }

    public static MasterTableView createAndBind(
            HasMasterVisualResultProperty pm, ControlFactoryMixin mixin) {
