...
public final class GroupView<E extends Entity>
    ...
    public void doDataBinding(HasGroupVisualResultProperty pm) {
        ...
        if (pm instanceof HasSelectedGroupProperty)
            bindWithSourceSelectedGroupProperty(
                    ((HasSelectedGroupProperty) pm).selectedGroupProperty());
        ...
        Node ui =
                tableOnly
                        ...
                        : new TabPane(
                                ...
                                createGroupTab(
                                        "pie", "images/s16/pieChart.png", new VisualPieChart()),
                                createGroupTab(
                                        "bar", "images/s16/barChart.png", new VisualBarChart()),
                                createGroupTab(
                                        "area", "images/s16/barChart.png", new VisualAreaChart()));
        ui.getProperties().put("groupView", this); // This is to avoid GC
        return ui;
    }

    private Tab createGroupTab(
            String text, String iconPath, SelectableVisualResultControl control) {
                    ...
                    .addListener(
                            (observable, oldValue, rs) ->
                                    control.setVisualResult(
                                            toSingleSeriesChartVisualResult(
                                                    rs, control instanceof VisualPieChart)));
        return control;
    }
    ...
    private VisualResult toSingleSeriesChartVisualResult(VisualResult rs, boolean pie) {
        ...
        if (rs != null) {
            ...
            for (int col = 0; col < colCount; col++) {
                ...
                // If not a number, it may be a formatted number (ex: Price), so checking if the
                // source is an expression column with a numeric type
                if (!isNumber
            ...
            VisualResultBuilder rsb =
                    new VisualResultBuilder(
                            rowCount,
                            new VisualColumnBuilder(null, PrimType.STRING)
                                    .setRole(pie ? "series" : null)
                                    .build(),
                            VisualColumn.create(null, PrimType.INTEGER));
            for (int row = 0; row < rowCount; row++) {
                // Generating the series name by concatenating text of all columns preceding the
                // value column
