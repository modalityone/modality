...
public class GroupMasterSlaveView extends MasterSlaveView {
  ...
  private final ObjectProperty<Node> groupViewProperty =
      new SimpleObjectProperty<Node /*GWT*/>() {
        @Override
  ...
  protected void updateView() {
    updateSplitPane(
        isGroupVisible() ? getGroupView() : null,
        isMasterVisible() ? getMasterView() : null,
        isSlaveVisible() ? getSlaveView() : null);
  }

  private final BooleanProperty groupVisibleProperty =
  ...
  public boolean isGroupVisible() {
    // Note: groupVisibleProperty is null on first call because not yet initialized (constructor ->
    // set master/slave view -> updateView() -> isGroupVisible()))
  ...
  private final BooleanProperty masterVisibleProperty =
  ...
  public boolean isMasterVisible() {
    // Note: masterVisibleProperty is null on first call because not yet initialized (constructor ->
    // set master/slave view -> updateView() -> isMasterVisible())
  ...
  public <E extends Entity> void doVisibilityBinding(
    ...
    groupVisibleProperty()
        .bind(
            FXProperties.compute(
                groupView.groupDqlStatementProperty(),
                statement -> statement != null && statement.getGroupBy() != null));
    masterVisibleProperty()
        .bind(
            FXProperties.combine(
                groupVisibleProperty(),
                groupView.selectedGroupProperty(),
                (groupVisible, selectedGroup) -> !groupVisible || selectedGroup != null));
    slaveVisibleProperty()
        .bind(
            FXProperties.combine(
                masterVisibleProperty(),
                masterSelectedEntityProperty,
                (masterVisible, selectedEntity) ->
                    masterVisible
                        && selectedEntity != null
                        && (additionalSlaveVisibilityCondition == null
                            || additionalSlaveVisibilityCondition.apply(selectedEntity))));
  }
  ...
  public static <
          ...
          PM extends
              HasGroupVisualResultProperty & HasMasterVisualResultProperty
                  & HasSelectedMasterProperty<E>>
      GroupMasterSlaveView createAndBind(
          PM pm, ControlFactoryMixin mixin, Supplier<Pane> containerGetter) {
    UiBuilder slaveView =
        MasterSlaveView.createAndBindSlaveViewIfApplicable(pm, mixin, containerGetter);
  ...
  public static <
          ...
          PM extends
              HasGroupVisualResultProperty & HasMasterVisualResultProperty
                  & HasSlaveVisualResultProperty & HasSelectedMasterProperty<E>>
  ...
  public static <
          ...
          PM extends
              HasGroupVisualResultProperty & HasMasterVisualResultProperty
                  & HasSelectedMasterProperty<E>>
      GroupMasterSlaveView createAndBind(
          UiBuilder slaveUiFactory, PM pm, ControlFactoryMixin mixin) {
    return createAndBind(MasterTableView.createAndBind(pm, mixin), slaveUiFactory, pm);
  }

  public static <
          E extends Entity, PM extends HasGroupVisualResultProperty & HasSelectedMasterProperty<E>>
      GroupMasterSlaveView createAndBind(
          UiBuilder masterUiFactory, UiBuilder slaveUiFactory, PM pm) {
    return createAndBind(GroupView.createAndBind(pm), masterUiFactory, slaveUiFactory, pm);
  }

  public static <E extends Entity> GroupMasterSlaveView createAndBind(
      GroupView<E> groupView,
      UiBuilder masterView,
      UiBuilder slaveView,
      HasSelectedMasterProperty<E> pm) {
    return createAndBind(
        groupView, masterView.buildUi(), slaveView == null ? null : slaveView.buildUi(), pm);
  }

  public static <E extends Entity> GroupMasterSlaveView createAndBind(
      GroupView<E> groupView, Node masterView, Node slaveView, HasSelectedMasterProperty<E> pm) {
    return createAndBind(
        groupView,
        masterView,
        slaveView,
        pm.selectedMasterProperty(),
        pm instanceof HasSlaveVisibilityCondition
            ? ((HasSlaveVisibilityCondition<E>) pm)::isSlaveVisible
            : null);
  }
  ...
  public static <E extends Entity> GroupMasterSlaveView createAndBind(
      ...
      ObjectProperty<E> masterSelectedEntityProperty,
      Function<E, Boolean> additionalSlaveVisibilityCondition) {
    GroupMasterSlaveView groupMasterSlaveView =
        new GroupMasterSlaveView(groupView.buildUi(), masterView, slaveView);
    groupMasterSlaveView.doVisibilityBinding(
        groupView, masterSelectedEntityProperty, additionalSlaveVisibilityCondition);
