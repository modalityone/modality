    ...
    return store
        .executeQueryBatch(
            new EntityStoreQuery(
                Strings.replaceAll(DOCUMENT_LINE_LOAD_QUERY, "document=?", documentCondition),
                parameter),
            new EntityStoreQuery(
                Strings.replaceAll(ATTENDANCE_LOAD_QUERY, "document=?", documentCondition),
                parameter),
            new EntityStoreQuery(
                Strings.replaceAll(PAYMENT_LOAD_QUERY, "document=?", documentCondition), parameter))
        .compose(
            entityLists -> {
              ...
              eventAggregate =
                  EventAggregate.getOrCreateFromDocument(cartDocumentLines.get(0).getDocument());
