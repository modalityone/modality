...
final class EventAggregateImpl implements EventAggregate {
  ...
  private static final Map<Object, EventAggregate> aggregates = new HashMap<>();
  ...
  static EventAggregate getOrCreate(Object eventId, DataSourceModel dataSourceModel) {
    EventAggregate service = get(eventId);
    if (service == null) service = getOrCreate(eventId, EntityStore.create(dataSourceModel));
    return service;
  }

  private static Object toKey(Object eventId) {
    eventId = Entities.getPrimaryKey(eventId);
    if (eventId instanceof Number) eventId = eventId.toString();
    return eventId;
  }
  ...
  public PersonAggregate getPersonAggregate() {
    if (personAggregate == null) personAggregate = PersonAggregate.getOrCreate(store);
  ...
  public Future<Event> onEvent() {
    if (eventId == null || getEvent() != null) return Future.succeededFuture(event);
    /*
    ...
    */
    if (eventFutureBroadcaster == null)
      eventFutureBroadcaster =
          new FutureBroadcaster<>(
              store
                  .executeListQuery(
                      EVENTS_LIST_ID, "select <frontend_loadEvent> from Event where id=?", eventId)
  ...
  public Future<EntityList<Option>> onEventOptions() {
    if (eventOptionsFutureBroadcaster == null) {
      ...
      String optionCondition =
          "event.(id=? and (host=null or host=? or ?='localhost')) and online and (!dev or ?=true)";
      String siteIds = "(select site.id from Option where " + optionCondition + ")";
      String rateCondition =
          "site.id in "
              + siteIds
              + " and (startDate is null or startDate <= site.event.endDate) and (endDate is null or endDate >= site.event.startDate) and (onDate is null or onDate <= now()) and (offDate is null or offDate > now())";
      eventOptionsFutureBroadcaster =
          new FutureBroadcaster<>(
              store
                  .executeQueryBatch(
                      new EntityStoreQuery(
                          "select <frontend_loadEvent> from Option where "
                              + optionCondition
                              + " order by ord",
                          parameters,
                          OPTIONS_LIST_ID),
                      new EntityStoreQuery(
                          "select <frontend_loadEvent> from Site where id in " + siteIds,
                          parameters,
                          SITES_LIST_ID),
                      new EntityStoreQuery(
                          "select <frontend_loadEvent> from Rate where " + rateCondition,
                          parameters,
                          RATES_LIST_ID),
                      new EntityStoreQuery(
                          "select <frontend_loadEvent> from DateInfo where event=? order by id",
                          new Object[] {eventId},
                          DATE_INFOS_LIST_ID))
  ...
  public Event getEvent() {
    if (event == null && eventId != null) {
      event =
          store.getEntity(
              "Event",
              eventId); // eventId may be from the wrong type (ex: String) because coming from the
                        // url
  ...
  public boolean hasUnemployedRate() {
    if (hasUnemployedRate == null)
      hasUnemployedRate =
          hasRate(
              rate -> Objects.anyNotNull(rate.getUnemployedPrice(), rate.getUnemployedDiscount()));
  ...
  public boolean hasFacilityFeeRate() {
    if (hasFacilityFeeRate == null)
      hasFacilityFeeRate =
          hasRate(
              rate ->
                  Objects.anyNotNull(rate.getFacilityFeePrice(), rate.getFacilityFeeDiscount()));
  ...
  public Future<QueryResult> onEventAvailabilities() {
    if (eventAvailabilitiesFutureBroadcaster == null)
      eventAvailabilitiesFutureBroadcaster =
          new FutureBroadcaster<>(
              () ->
                          ...
                          QueryArgument.builder()
                              .setStatement(
                                  "with ra as (select * from resource_availability_by_event_items(?) where max>0),"
                                      + // resources with max(=max_online)=0 (like private rooms)
                                        // are not displayed in the front-office
                                      // let's see if some options for this event require to have
                                      // the per day availabilities details
                                      " pda as (select site_id,item_id,item_family_id from option where per_day_availability and event_id=?)"
                                      +
                                      // for such options we keep all the details: site, item and
                                      // date (this applies to availabilities having
                                      // site=option.site and item=option.item if set,
                                      // item_family=item.family otherwise)
                                      ...
                                      // for others, we group by site and item (=> dates disappears
                                      // => simpler and less data to transfer to browser) and keep
                                      // the min values for availability all over the event time
                                      // range
