        ...
        if (eventFutureBroadcaster == null)
            eventFutureBroadcaster =
                    new FutureBroadcaster<>(
                            store.executeListQuery(
                                            EVENTS_LIST_ID,
                                            "select <frontend_loadEvent> from Event where id=?",
                                            eventId)
                    ...
                    new FutureBroadcaster<>(
                            store.executeQueryBatch(
                                            new EntityStoreQuery(
                                                    ...
                                                    OPTIONS_LIST_ID),
                                            new EntityStoreQuery(
                                                    "select <frontend_loadEvent> from Site where id in "
                                                            + siteIds,
                                                    parameters,
                                                    SITES_LIST_ID),
                                            new EntityStoreQuery(
                                                    "select <frontend_loadEvent> from Rate where "
                                                            + rateCondition,
                                                    parameters,
                                                    RATES_LIST_ID),
                                            new EntityStoreQuery(
        ...
        if (hasUnemployedRate == null)
            hasUnemployedRate =
                    hasRate(
                            rate ->
                                    Objects.anyNotNull(
                                            rate.getUnemployedPrice(),
                                            rate.getUnemployedDiscount()));
        ...
        if (hasFacilityFeeRate == null)
            hasFacilityFeeRate =
                    hasRate(
                            rate ->
                                    Objects.anyNotNull(
                                            rate.getFacilityFeePrice(),
                                            rate.getFacilityFeeDiscount()));
