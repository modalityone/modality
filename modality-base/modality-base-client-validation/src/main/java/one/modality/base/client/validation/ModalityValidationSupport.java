...
public final class ModalityValidationSupport {
    ...
    public void addRequiredInput(
            ObservableValue valueProperty, Node inputNode, String errorMessage) {
        addValidationRule(
                Bindings.createBooleanBinding(
                        () -> testNotEmpty(valueProperty.getValue()), valueProperty),
                inputNode,
                errorMessage);
    }
    ...
    public void addValidationRule(
        ...
        ObservableBooleanValue rule =
                Bindings.createBooleanBinding(
                        () ->
                                !validatingProperty.get()
                                        || validProperty.getValue()
                                        || !isShowing(node),
                        validProperty,
                        validatingProperty);
                    ...
                    new GraphicValidationDecoration() {
                        ...
                        protected Node createErrorNode() {
                            return ImageStore.createImageView(
                                    ModalityValidationIcons.validationErrorIcon16Url);
                        }

                        @Override
                        protected Collection<Decoration> createValidationDecorations(
                                dev.webfx.stack.ui.validation.controlsfx.validation
                                                .ValidationMessage
                                        message) {
                            boolean isTextInput = node instanceof TextInputControl;
                            boolean isButton = node instanceof Button;
                            // isInside flag will determine if we position the decoration inside the
                            // node or not (ie outside)
                            boolean isInside;
                            ...
                            return java.util.Collections.singletonList(
                                    new GraphicDecoration(
                                            ImageStore.createImageView(
                                                    ModalityValidationIcons
                                                            .validationRequiredIcon16Url),
                                            Pos.CENTER_LEFT,
                                            -10,
                                            0));
                        }
                ...
                () -> {
                    ...
                    if (isShowing(popOverOwnerNode)) {
                        ...
                        if (popUpOverAutoScroll) {
                            SceneUtil.scrollNodeToBeVerticallyVisibleOnScene(
                                    popOverDecorationTarget);
