...
public final class ModalityValidationSupport {
  ...
  private Validator firstInvalidValidator() {
    return Collections.findFirst(
        validators, validator -> !validator.getValidationStatus().isValid());
  }

  public void addRequiredInputs(TextInputControl... textInputControls) {
    for (TextInputControl textInputControl : textInputControls) addRequiredInput(textInputControl);
  }
  ...
  public void addRequiredInput(ObservableValue valueProperty, Node inputNode, String errorMessage) {
    addValidationRule(
        Bindings.createBooleanBinding(() -> testNotEmpty(valueProperty.getValue()), valueProperty),
        inputNode,
        errorMessage);
  }
  ...
  public void addValidationRule(
      ObservableValue<Boolean> validProperty, Node node, String errorMessage) {
    ...
    if (node instanceof Control) {
      ...
      validationVisualizer.setDecoration(
          new GraphicValidationDecoration() {
            ...
            protected Collection<Decoration> createValidationDecorations(
                dev.webfx.stack.ui.validation.controlsfx.validation.ValidationMessage message) {
              boolean isTextInput = node instanceof TextInputControl;
              boolean isButton = node instanceof Button;
              // isInside flag will determine if we position the decoration inside the node or not
              // (ie outside)
              boolean isInside;
              if (isTextInput) // inside for text inputs
              isInside = true;
              else { // for others, will be generally outside unless it is stretched to full width
                     // by its container
                Parent parent = node.getParent();
                while (parent instanceof Pane
                    && !(parent instanceof VBox)
                    && !(parent instanceof HBox)) parent = parent.getParent();
                isInside = parent instanceof VBox && ((VBox) parent).isFillWidth();
              }
              double xRelativeOffset =
                  isInside
                      ? -1
                      : 1; // positioning the decoration inside the control for button and text
                           // input
              double xOffset =
                  isInside && isButton
                      ? -20
                      : 0; // moving the decoration before the drop down arrow
            ...
            protected Collection<Decoration> createRequiredDecorations(Control target) {
              return java.util.Collections.singletonList(
                  new GraphicDecoration(
                      ImageStore.createImageView(
                          ModalityValidationIcons.validationRequiredIcon16Url),
                      Pos.CENTER_LEFT,
                      -10,
                      0));
            }
  ...
  private void showValidatorErrorPopOver(Validator validator) {
    ...
    if (index >= 0) {
      ...
      if (decorationNode != null) showValidatorErrorPopOver(validator, decorationNode);
    }
  }

  private void showValidatorErrorPopOver(Validator validator, Node errorDecorationNode) {
    ValidationMessage errorMessage =
        Collections.first(validator.getValidationStatus().getErrorMessages());
    if (errorMessage != null) {
      ...
      diamond
          .layoutYProperty()
          .bind(FXProperties.compute(label.heightProperty(), n -> n.doubleValue() - 7));
  ...
  */
  ...
  private static boolean isShowing(Node node) {
    ...
    Parent parent = node.getParent();
    if (parent != null) return isShowing(parent);
