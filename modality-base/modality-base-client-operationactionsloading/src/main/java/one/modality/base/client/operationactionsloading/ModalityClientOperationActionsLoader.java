 ...
 * @author Bruno Salmon
 */
public class ModalityClientOperationActionsLoader
        implements ApplicationModuleBooter, OperationActionFactoryMixin, ActionFactoryMixin {
    ...
    public void bootModule() {
        // Temporary load only the operations for the backend (ie back-office) for the demo (because
        // backend and frontend fields are not considered by authz so far)
                            ...
                            for (Entity operation : operations) {
                                ...
                                Action operationGraphicalAction =
                                        isPublic
                                                ...
                                                : newAuthAction(
                                                        i18nKey,
                                                        registry.authorizedOperationActionProperty(
                                                                operationCode,
                                                                AuthorizationFactory
                                                                        ::isAuthorized));
                                operationGraphicalAction.setUserData(i18nKey);
                                registry.registerOperationGraphicalAction(
                                        operationCode, operationGraphicalAction);
                            }
                            // Telling the registry how to update the graphical properties when
                            // needed (ex: ToggleCancel actions text
                            // needs to be updated to say 'Cancel' or 'Uncancel' on selection
                            // change)
                            registry.setOperationActionGraphicalPropertiesUpdater(
                                    operationAction -> {
                                        // Actually since text and graphic properties come from
                                        // I18n, we just need to inform it about the
                                        // change, and it will refresh all translations, including
                                        // therefore these graphical properties.
                                        // The possible expressions used by operations like
                                        // ToggleCancel will be recomputed through this
                                        // refresh thanks to the I18n evaluation system.
                                        // Instantiating an operation request just to have the
                                        // request class or operation code
                                        Object operationRequest =
                                                registry.newOperationActionRequest(operationAction);
                                        // Then getting the graphical action from it
                                        Action graphicalAction =
                                                registry.getGraphicalActionFromOperationRequest(
                                                        operationRequest);
                                        if (graphicalAction != null) {
                                            Object i18nKey = graphicalAction.getUserData();
                                            if (i18nKey instanceof ModalityOperationI18nKey)
                                                ((ModalityOperationI18nKey) i18nKey)
