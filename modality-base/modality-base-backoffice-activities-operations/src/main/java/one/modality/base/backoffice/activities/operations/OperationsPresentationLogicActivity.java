...
final class OperationsPresentationLogicActivity
    ...
    private final String expressionColumns =
            "['name','operationCode','i18nCode','backend','frontend','public']";

    OperationsPresentationLogicActivity() {
        this(OperationsPresentationModel::new);
    }

    private OperationsPresentationLogicActivity(
            Factory<OperationsPresentationModel> presentationModelFactory) {
    ...
    protected void startLogic(OperationsPresentationModel pm) {
        ReactiveVisualMapper.createPushReactiveChain(this)
                ...
                .ifTrimNotEmpty(
                        ...
                        s -> where("lower(operationCode) like ?", "%" + s.toLowerCase() + "%"))
                // Limit condition
