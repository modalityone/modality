    ...
    LuminanceTheme.createApplicationFrameFacet(containerHeader)
        .setOnMouseClicked(
            e -> { // Temporary for testing
              if (e.isAltDown()) FXPaletteMode.setVariedPalette(!FXPaletteMode.isVariedPalette());
              if (e.isShiftDown()) FXLuminanceMode.setDarkMode(!FXLuminanceMode.isDarkMode());
              if (e.isMetaDown()) FXLayoutMode.setCompactMode(!FXLayoutMode.isCompactMode());
            })
        .style();
    // Hiding the center item in compact mode
    FXLayoutMode.layoutModeProperty()
        .addListener(observable -> headerCenterItem.setVisible(!FXLayoutMode.isCompactMode()));
