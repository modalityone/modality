...
public class ModalityClientApplication extends Application {
    ...
    public void init() {
        ActivityManager.runActivity(
                modalityClientStarterActivity,
                ViewDomainActivityContext.createViewDomainActivityContext(
        ...
        scene.rootProperty()
                .bind(
                        FXProperties.compute(
                                modalityClientStarterActivity.nodeProperty(), n -> (Parent) n));
                ...
                if (spinner == null) {
                    spinner =
                            ImageStore.createImageView(
                                    "one/modality/base/client/images/spinner.gif");
                ...
                spinner.xProperty()
                        .bind(
                                FXProperties.combine(
                                        ...
                                        spinner.getImage().widthProperty(),
                                        (w1, w2) -> (w1.doubleValue() - w2.doubleValue()) / 2));
                spinner.yProperty()
                        .bind(
                                FXProperties.combine(
                                        rootPane.heightProperty(),
                                        spinner.getImage().heightProperty(),
                                        (h1, h2) -> (h1.doubleValue() - h2.doubleValue()) / 2));
