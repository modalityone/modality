...
public class ModalityClientApplication extends Application {
  ...
  public void start(Stage primaryStage) {
    ...
    scene
        .rootProperty()
        .bind(FXProperties.compute(modalityClientStarterActivity.nodeProperty(), n -> (Parent) n));
  ...
  private static void setLoadingSpinnerVisibleConsumer(Consumer<Boolean> consumer) {
    consumeInUiThread(
        FXProperties.compute(
            PendingBusCall.pendingCallsCountProperty(), pendingCallsCount -> pendingCallsCount > 0),
        consumer);
  }
