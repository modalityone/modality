...
import static one.modality.hotel.shared.businessdata.time.TimeConverter.convertTime;
...
public class DaysArray implements Iterable<Long> {
  ...
  public Iterator<Long> iterator() {
    return new Iterator<Long>() {
      ...
      public void
          remove() { // necessary for GWT compilation (perhaps they haven't written the default
                     // method)
  ...
  public DaysArray changeTimeUnit(TimeUnit newTimeUnit) {
    if (timeUnit == newTimeUnit) return this;
    int n = array.length, i = 0;
    long[] newArray = new long[n];
    for (long day : array) newArray[i++] = convertTime(day, timeUnit, newTimeUnit);
  ...
  public DaysArray shift(long shiftDays, TimeUnit shiftTimeUnit) {
    ...
    if (shiftDays == 0) return this;
    int n = array.length, i = 0;
    long[] newArray = new long[n];
    for (long day : array) newArray[i++] = day + shiftDays;
  ...
  public boolean overlaps(DaysArray daysArray) {
    if (!daysArray.isEmpty()) {
      ...
      for (long day : array) if (daysArray.indexOfDay(day) != -1) return true;
    }
    return false;
  }
