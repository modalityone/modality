...
public final class TimeSeries {
  ...
  public TimeSeries shift(long offset, TimeUnit timeUnit) {
    ...
    return new TimeSeries(
        Arrays.convert(
            array, interval -> interval.shift(finalOffset, finalUnit), TimeInterval[]::new),
        finalUnit);
  }
  ...
  StringBuilder toText(StringBuilder sb, String format) {
    int index = 0;
    for (TimeInterval interval : array) interval.toText(index++ > 0 ? sb.append(", ") : sb, format);
    return sb;
  }
  ...
  private static TimeSeriesBuilder intervalIntersectDayTimeRange(
    ...
    TimeInterval endDayTimeInterval =
        dayTimeRange.getDayTimeInterval(endDay, TimeUnit.MINUTES).shift(endDay, TimeUnit.MINUTES);
  ...
  public static TimeSeries merge(Collection<TimeSeries> seriesCollection) {
    ...
    Collections.forEach(
        seriesCollection,
        series ->
            java.util.Collections.addAll(intervals, series.changeTimeUnit(timeUnit).getArray()));
