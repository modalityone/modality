...
public final class TimeSeries {
    ...
    public TimeSeries changeTimeUnit(TimeUnit newTimeUnit) {
        ...
        for (TimeInterval interval : array) newArray[i++] = interval.changeTimeUnit(newTimeUnit);
    ...
    public TimeSeries shift(long offset, TimeUnit timeUnit) {
        ...
        return new TimeSeries(
                Arrays.convert(
                        array,
                        interval -> interval.shift(finalOffset, finalUnit),
                        TimeInterval[]::new),
                finalUnit);
    }
    ...
    public TimeInterval toInterval() {
        ...
        if (n >= 2)
            return new TimeInterval(
                    array[0].getIncludedStart(), array[n - 1].getExcludedEnd(), timeUnit);
    ...
    public TimeSeries intersect(DayTimeRange dayTimeRange) {
        return seriesIntersectDayTimeRange(this, dayTimeRange, new TimeSeriesBuilder(timeUnit))
                .build();
    }
    ...
    private static TimeSeriesBuilder seriesIntersectSeries(
            TimeSeries series1, TimeSeries series2, TimeSeriesBuilder tsb) {
    ...
    private static TimeSeriesBuilder seriesIntersectInterval(
            TimeSeries series, TimeInterval interval, TimeSeriesBuilder tsb) {
        // loop but could be optimized knowing that intervals are in chronological order
    ...
    private static TimeSeriesBuilder intervalIntersectInterval(
            TimeInterval interval1, TimeInterval interval2, TimeSeriesBuilder tsb) {
        ...
        if (start < end) tsb.addInterval(new TimeInterval(start, end, timeUnit));
        return tsb;
    }

    private static TimeSeriesBuilder seriesIntersectDayTimeRange(
            TimeSeries series, DayTimeRange dayTimeRange, TimeSeriesBuilder tsb) {
        // loop but could be optimized knowing that intervals are in chronological order
        for (TimeInterval interval1 : series.getArray())
    ...
    private static TimeSeriesBuilder intervalIntersectDayTimeRange(
            TimeInterval interval, DayTimeRange dayTimeRange, TimeSeriesBuilder tsb) {
        ...
        TimeInterval startDayTimeInterval =
                dayTimeRange
                        .getDayTimeInterval(startDay, TimeUnit.MINUTES)
                        .shift(startDay, TimeUnit.MINUTES);
        TimeInterval endDayTimeInterval =
                dayTimeRange
                        .getDayTimeInterval(endDay, TimeUnit.MINUTES)
                        .shift(endDay, TimeUnit.MINUTES);
        if (start < startDayTimeInterval.getIncludedStart())
            ...
            start =
                    startDayTimeInterval.getIncludedStart()
                            + TimeConverter.oneDay(TimeUnit.MINUTES);
        if (end > endDayTimeInterval.getExcludedEnd()) end = endDayTimeInterval.getExcludedEnd();
        ...
        if (start < end)
            intervalIntersectInterval(
                    new TimeInterval(start, end, TimeUnit.MINUTES), interval, tsb);
        return tsb;
    }

    public static TimeSeries merge(Collection<TimeSeries> seriesCollection) {
        TimeUnit timeUnit =
                seriesCollection.stream()
                        .min(Collections.comparing(TimeSeries::getTimeUnit))
                        .get()
                        .getTimeUnit();
        ...
        Collections.forEach(
                ...
                series ->
                        java.util.Collections.addAll(
                                intervals, series.changeTimeUnit(timeUnit).getArray()));
        ...
        for (TimeInterval interval : intervals) tsb.addInterval(interval);
        return tsb.build();
    }
}
