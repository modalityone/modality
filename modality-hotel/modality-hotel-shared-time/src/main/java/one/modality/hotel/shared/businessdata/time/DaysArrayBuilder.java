...
public class DaysArrayBuilder {
  ...
  public DaysArrayBuilder addInterval(TimeInterval interval, DayTimeRange dayTimeRange) {
    ...
    if (dayTimeRange != null) {
      if (firstDayTime
          >= dayTimeRange.getDayTimeInterval(includedFirstDay, intervalTimeUnit).getExcludedEnd())
        includedFirstDay += oneDay;
      if (lastDayTime
          < dayTimeRange.getDayTimeInterval(excludedLastDay, intervalTimeUnit).getIncludedStart())
