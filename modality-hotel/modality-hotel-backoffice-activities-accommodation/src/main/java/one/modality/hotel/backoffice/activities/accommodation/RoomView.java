        ...
        scheduledResourceGantt =
                new ScheduledResourceGantt(
                    ...
                    protected void drawBar(
                            LocalDateBar<ScheduledResourceBlock> bar,
                            Bounds b,
                            GraphicsContext gc) {
                        // The bar wraps a block over 1 or several days (or always 1 day if the user
                        // hasn't ticked the grouping block
                        // checkbox). So the bar instance is that block that was repeated over that
                        // period.
                        ScheduledResourceBlock block = bar.getInstance();
                        // The main info we display in the bar is a number which represents how many
                        // free beds are remaining for booking
                        String remaining = String.valueOf(block.getRemaining());
                        // If the bar is wide enough we show "Beds" on top and the number on bottom,
                        // but if it is too narrow, we just
                        // display the number in the middle. Unavailable gray bars have no text at
                        // all by the way.
