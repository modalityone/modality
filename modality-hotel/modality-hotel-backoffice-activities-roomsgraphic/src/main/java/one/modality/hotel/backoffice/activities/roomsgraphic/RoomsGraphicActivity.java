...
final class RoomsGraphicActivity extends EventDependentViewDomainActivity
        implements HasSelectedDocumentProperty, OperationActionFactoryMixin {

    private static final String ITEM_FAMILY_CODE = "acco";
    ...
    private Pane
            container; // Keeping a reference to the container to act as parent for dialog windows
    ...
    public Node buildUi() {
        ...
        sitesToTabsMapper =
                ReactiveObjectsMapper.<Site, Tab>createPushReactiveChain(this)
                        .always(
                        ...
                        .start();
        MasterSlaveView masterSlaveView =
                new MasterSlaveView(
                        sitesTabPane,
                        MasterSlaveView.createAndBindSlaveViewIfApplicable(
                                        this, this, () -> container)
                                .buildUi());
        masterSlaveView
                .slaveVisibleProperty()
                .bind(FXProperties.compute(selectedDocumentProperty(), Objects::nonNull));
    ...
    class IndividualSiteToTabMapper implements IndividualEntityToObjectMapper<Site, Tab> {
        ...
        IndividualSiteToTabMapper(Site site) {
            ...
            // Mapping the site items to tabs. Each site item will be represented by a
            // ResourceConfiguration group
            // (of this site and one of its possible items)
            siteItemsToTabsMapper =
                    ReactiveObjectsMapper.<Entity, Tab>createPushReactiveChain(
                                    RoomsGraphicActivity.this)
                            .always(
                            ...
                            .ifNotNullOtherwiseEmpty(
                                    ...
                                    s ->
                                            where(
                                                    "resource.site=? and item.family.code=?",
                                                    s,
                                                    ITEM_FAMILY_CODE))
                            .setActiveParent(sitesToTabsMapper)
                            .bindActivePropertyTo(siteTab.selectedProperty())
                            .setIndividualEntityToObjectMapperFactory(
                                    IndividualSiteItemToTabMapper::new)
        ...
        class IndividualSiteItemToTabMapper implements IndividualEntityToObjectMapper<Entity, Tab> {
            // The site item is represented by a ResourceConfiguration group (having this site and
            // item)
            final ObjectProperty<Entity> siteItemProperty = new SimpleObjectProperty<>();
            final Pane boxesContainer = new FlexBox(10, 10);
            final Tab siteItemTab =
                    new Tab(null, LayoutUtil.createVerticalScrollPane(boxesContainer));
            final ReactiveObjectsMapper<ResourceConfiguration, Node>
                    siteItemResourceConfigurationsToBoxesMapper;

            IndividualSiteItemToTabMapper(
                    Entity siteItem) { // actually a ResourceConfiguration group
                boxesContainer.setPadding(new Insets(10));
                siteItemTab.setClosable(false);
                onEntityChangedOrReplaced(siteItem);
                // Mapping the site item resource (configurations) to boxes. Each
                // ResourceConfiguration is an individual
                // member of the site item ResourceConfiguration group (so all having the same site
                // and item).
                siteItemResourceConfigurationsToBoxesMapper =
                        ReactiveObjectsMapper.<ResourceConfiguration, Node>create(
                                        ReactiveEntitiesMapper.createPushReactiveChain(
                                                RoomsGraphicActivity.this))
                                ...
                                .ifNotNullOtherwiseEmpty(
                                        ...
                                        rc ->
                                                where(
                                                        "resource.site=? and item=?",
                                                        rc.evaluate("resource.site"),
                                                        rc.evaluate("item")))
                                .bindActivePropertyTo(siteItemTab.selectedProperty())
                                .setIndividualEntityToObjectMapperFactory(
                                        IndividualSiteItemResourceConfigurationToBoxMapper::new)
            ...
            @Override
            public void onEntityChangedOrReplaced(
                    Entity siteItem) { // actually a ResourceConfiguration group
                siteItemProperty.set(siteItem);
                siteItemTab.setText((String) siteItem.evaluate("item.name"));
                siteItemTab.setGraphic(
                        ImageStore.createImageView((String) siteItem.evaluate("item.icon")));
            }
            ...
            class IndividualSiteItemResourceConfigurationToBoxMapper
                    implements IndividualEntityToObjectMapper<ResourceConfiguration, Node> {

                private final ObjectProperty<ResourceConfiguration>
                        siteItemResourceConfigurationProperty = new SimpleObjectProperty<>();
                private final Label label = new Label();
                private final VisualGrid peopleGrid = new SkinnedVisualGrid();
                private final VBox box =
                        new VBox(LayoutUtil.setMaxWidthToInfinite(label), peopleGrid);
                private final ReactiveVisualMapper<DocumentLine> peopleVisualMapper;
                private boolean dragBackgroundVisible;

                IndividualSiteItemResourceConfigurationToBoxMapper(
                        ResourceConfiguration siteItemResourceConfiguration) {
                    ...
                    // Visualizing people booked for that site item resource configuration into a
                    // visual grid (inside the box)
                    // Each person (row in the grid) is represented by a DocumentLine allocated to
                    // that site item resource configuration
                    peopleVisualMapper =
                            ReactiveVisualMapper.<DocumentLine>createPushReactiveChain(
                                            RoomsGraphicActivity.this)
                                    ...
                                    .ifNotNullOtherwiseEmpty(
                                            siteItemResourceConfigurationProperty,
                                            rc -> where("resourceConfiguration=?", rc))
                                    // Setting the aggregate scope TODO Can this be automatically
                                    // set by the Dql query push interceptor (or
                                    // QueryInfo.getQueryScope())?
                                    .setAggregateScope(
                                            siteItemResourceConfigurationProperty,
                                            rc ->
                                                    AggregateScope.builder()
                                                            .addAggregate(
                ...
                public void onEntityChangedOrReplaced(
                    ...
                    box.setBackground(
                            dragBackgroundVisible
                                    ? DRAG_BOX_BACKGROUND
                                    : online ? ONLINE_BOX_BACKGROUND : OFFLINE_BOX_BACKGROUND);
                    box.setBorder(BOX_BORDER);
                    peopleGrid.setBackground(
                            online ? ONLINE_PEOPLE_BACKGROUND : OFFLINE_PEOPLE_BACKGROUND);
                }
                ...
                ActionGroup createContextMenuActionGroup() {
                    return newActionGroup(
                            newOperationAction(
                                    () ->
                                            new EditResourceConfigurationPropertiesRequest(
                                                    getSiteItemResourceConfiguration(), container)),
                            newOperationAction(
                                    () ->
                                            new ToggleResourceConfigurationOnlineOfflineRequest(
                                                    getSiteItemResourceConfiguration())),
                            newOperationAction(
                                    () ->
                                            new ChangeResourceConfigurationItemRequest(
                                                    getSiteItemResourceConfiguration(),
                                                    container,
                                                    "acco",
                                                    siteProperty.get().getId())),
                            newOperationAction(
                                    () ->
                ...
                private void setUpDragAndDropSupport() {
                    ...
                    box.setOnDragOver(
                            e -> {
                                if (acceptsDrag(e)) e.acceptTransferModes(TransferMode.MOVE);
                            });
                    box.setOnDragDropped(
                            e -> {
                                Dragboard db = e.getDragboard();
                                if (db.hasContent(dndDataFormat)) {
                                    Object[] keys =
                                            importDragSelectionFromDragboard(
                                                    db.getContent(dndDataFormat));
                                    executeOperation(
                                            new MoveToResourceConfigurationRequest(
                                                    getSiteItemResourceConfiguration(), keys));
                                }
                            });
                ...
                private boolean acceptsDrag(DragEvent e) {
                    return e.getGestureSource() != peopleGrid
                            && e.getDragboard().hasContent(dndDataFormat);
                }
                ...
                private String exportDragSelectionForDragboard() {
                    ...
                    return SerialCodecManager.encodePrimitiveArrayToJsonArray(
                                    documentLines.stream().map(Entity::getPrimaryKey).toArray())
                            .toJsonString();
                }

                private Object[] importDragSelectionFromDragboard(Object dragContent) {
                    // Parsing the expected json array and decoding it as a java array containing
                    // all primary keys
                    return SerialCodecManager.decodePrimitiveArrayFromJsonArray(
                            Json.parseArray(dragContent.toString()));
                }
            }
        }
    }
    ...
    private static final CornerRadii BOX_RADII = new CornerRadii(8);
    private static final Color ONLINE_BOX_COLOR = Color.color(0, 0.8, 0);
    private static final Color OFFLINE_BOX_COLOR = Color.ORANGE;
    private static final Color DRAG_BOX_COLOR = Color.BLUEVIOLET;
    private static final Border BOX_BORDER =
            new Border(new BorderStroke(Color.GRAY, BorderStrokeStyle.SOLID, BOX_RADII, null));
    private static final Effect BOX_SHADOW_EFFECT = new DropShadow(5, 7, 7, Color.GRAY);
    private static final Background ONLINE_BOX_BACKGROUND =
            new Background(new BackgroundFill(ONLINE_BOX_COLOR, BOX_RADII, null));
    private static final Background ONLINE_PEOPLE_BACKGROUND =
            new Background(new BackgroundFill(ONLINE_BOX_COLOR.brighter(), null, null));
    private static final Background OFFLINE_BOX_BACKGROUND =
            new Background(new BackgroundFill(OFFLINE_BOX_COLOR, BOX_RADII, null));
    private static final Background OFFLINE_PEOPLE_BACKGROUND =
            new Background(new BackgroundFill(OFFLINE_BOX_COLOR.brighter(), null, null));
    private static final Background DRAG_BOX_BACKGROUND =
