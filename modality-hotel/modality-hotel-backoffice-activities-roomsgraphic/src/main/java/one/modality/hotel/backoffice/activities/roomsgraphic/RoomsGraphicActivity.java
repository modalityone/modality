...
final class RoomsGraphicActivity extends EventDependentViewDomainActivity
  ...
  public Node buildUi() {
    ...
    MasterSlaveView masterSlaveView =
        new MasterSlaveView(
            sitesTabPane,
            MasterSlaveView.createAndBindSlaveViewIfApplicable(this, this, () -> container)
                .buildUi());
    masterSlaveView
  ...
  class IndividualSiteToTabMapper implements IndividualEntityToObjectMapper<Site, Tab> {
    ...
    IndividualSiteToTabMapper(Site site) {
      ...
      siteItemsToTabsMapper =
          ReactiveObjectsMapper.<Entity, Tab>createPushReactiveChain(RoomsGraphicActivity.this)
              .always(
              ...
              .ifNotNullOtherwiseEmpty(
                  siteProperty,
                  s -> where("resource.site=? and item.family.code=?", s, ITEM_FAMILY_CODE))
    ...
    class IndividualSiteItemToTabMapper implements IndividualEntityToObjectMapper<Entity, Tab> {
      ...
      IndividualSiteItemToTabMapper(Entity siteItem) { // actually a ResourceConfiguration group
        ...
        // Mapping the site item resource (configurations) to boxes. Each ResourceConfiguration is
        // an individual
        // member of the site item ResourceConfiguration group (so all having the same site and
        // item).
        siteItemResourceConfigurationsToBoxesMapper =
            ReactiveObjectsMapper.<ResourceConfiguration, Node>create(
                    ReactiveEntitiesMapper.createPushReactiveChain(RoomsGraphicActivity.this))
      ...
      class IndividualSiteItemResourceConfigurationToBoxMapper
        ...
        private final ObjectProperty<ResourceConfiguration> siteItemResourceConfigurationProperty =
        ...
        IndividualSiteItemResourceConfigurationToBoxMapper(
          ...
          // Visualizing people booked for that site item resource configuration into a visual grid
          // (inside the box)
          // Each person (row in the grid) is represented by a DocumentLine allocated to that site
          // item resource configuration
          peopleVisualMapper =
              ReactiveVisualMapper.<DocumentLine>createPushReactiveChain(RoomsGraphicActivity.this)
                  ...
                  // Setting the aggregate scope TODO Can this be automatically set by the Dql query
                  // push interceptor (or QueryInfo.getQueryScope())?
                  .setAggregateScope(
                      siteItemResourceConfigurationProperty,
                      rc ->
                          AggregateScope.builder()
                              .addAggregate("ResourceConfiguration", rc.getPrimaryKey())
                              .build())
                  .unbindActiveProperty() // always active (under parent), no need for additional
                                          // (and costly) active calls
                  ...
                  .setSelectedEntityHandler(
                      dl -> {
                        ...
                        } else if (peopleGrid == peopleGridFocus) setSelectedDocument(null);
                      })
                  .start();
        ...
        ActionGroup createContextMenuActionGroup() {
          return newActionGroup(
              ...
              newOperationAction(
                  () -> new DeleteResourceRequest(getSiteItemResourceConfiguration(), container)));
        }
        ...
        private Object[] importDragSelectionFromDragboard(Object dragContent) {
          // Parsing the expected json array and decoding it as a java array containing all primary
          // keys
  ...
  private static final DataFormat dndDataFormat =
      DataFormat
          .PLAIN_TEXT; // Using standard plain text format to ensure drag & drop works between
                       // applications
}
