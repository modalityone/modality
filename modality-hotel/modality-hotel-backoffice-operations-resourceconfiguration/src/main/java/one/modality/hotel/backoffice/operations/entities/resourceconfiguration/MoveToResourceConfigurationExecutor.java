    ...
    private static Future<Void> execute(
        ...
        for (Object primaryKey : documentLinePrimaryKeys) {
            DocumentLine documentLine =
                    updateStore.getOrCreateEntity(DocumentLine.class, primaryKey);
            updateStore.updateEntity(documentLine).setResourceConfiguration(resourceConfiguration);
        }
        // Commented as now automatically set by the Dql submit interceptor TODO Remove this comment
        // once the feature is completed
        // updateStore.setSubmitScope(AggregateScope.builder().addAggregate("ResourceConfiguration",
        // resourceConfiguration.getPrimaryKey()).build());
        updateStore
                .submitChanges()
