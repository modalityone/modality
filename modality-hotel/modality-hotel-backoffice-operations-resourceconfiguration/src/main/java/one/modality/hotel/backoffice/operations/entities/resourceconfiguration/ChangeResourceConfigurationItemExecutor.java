...
final class ChangeResourceConfigurationItemExecutor {
    ...
    static Future<Void> executeRequest(ChangeResourceConfigurationItemRequest rq) {
        return execute(
                rq.getResourceConfiguration(),
                rq.getParentContainer(),
                rq.getItemFamilyCode(),
                rq.getSiteId());
    }
    ...
    private static Future<Void> execute(
        ...
        EntityButtonSelector<Item> itemSelector =
                new EntityButtonSelector<>(
                        "{class: `Item`, alias: `i`, where: `family.code='"
                                + itemFamilyCode
                                + "' and exists(select Option where item=i and site="
                                + siteId.getPrimaryKey()
                                + ")`, orderBy: `ord,id`}",
                ...
                () -> {
                    ...
                    if (selectedItem != null) {
                        DialogContent dialogContent =
                                new DialogContent().setContent(new Text("Are you sure?"));
                        DialogUtil.showModalNodeInGoldLayout(dialogContent, parentContainer)
                                .addCloseHook(
                                        () -> {
                                            if (!promise.future().isComplete()) promise.complete();
                                        });
                        DialogUtil.armDialogContentButtons(
                                ...
                                dialogCallback -> {
                                    ...
                                    updateStore
                                            .updateEntity(resourceConfiguration)
