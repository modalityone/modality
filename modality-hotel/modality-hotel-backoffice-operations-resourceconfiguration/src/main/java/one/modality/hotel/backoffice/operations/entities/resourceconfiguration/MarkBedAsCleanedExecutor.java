...
final class MarkBedAsCleanedExecutor {
  ...
  static Future<Void> executeRequest(MarkBedAsCleanedRequest rq) {
    return execute(
        rq.getRoomConfiguration(), rq.getBedIndex(), rq.getCleaningDate(), rq.getParentContainer());
  }
  ...
  private static Future<Void> execute(
    ...
    DialogContent dialogContent =
        new DialogContent()
            .setContent(
                new Text(
                    "Are you sure you want to mark bed "
                        + roomConfiguration.evaluate("name")
                        + " - "
                        + (bedIndex + 1)
                        + " as cleaned?"));
    ...
    DialogUtil.armDialogContentButtons(
        ...
        dialogCallback -> {
          UpdateStore updateStore =
              UpdateStore.create(roomConfiguration.getStore().getDataSourceModel());
