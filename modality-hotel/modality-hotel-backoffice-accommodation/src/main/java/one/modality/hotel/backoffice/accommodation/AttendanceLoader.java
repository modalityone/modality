...
public final class AttendanceLoader {
  ...
  // entities mappers (ie
  // sending the exact same query and parameters to the server) run on the same client => the issue
  // is that the push
          ...
          ReactiveEntitiesMapper.<Attendance>createPushReactiveChain(mixin)
              ...
              .always(
                  ...
                  startDate ->
                      where(
                          "a.date +1 >= ?",
                          startDate)) // +1 is to avoid the round corners on left  for bookings
                                      // exceeding the time window
              .always(
                  pm.timeWindowEndProperty(),
                  endDate ->
                      where(
                          "a.date -1 <= ?",
                          endDate)) // -1 is to avoid the round corners on right for bookings
                                    // exceeding the time window
