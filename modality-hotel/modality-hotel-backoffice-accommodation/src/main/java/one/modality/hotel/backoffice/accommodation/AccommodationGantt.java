...
public abstract class AccommodationGantt<B extends AccommodationBlock> {
   ...
   * instance per day), TimeBarUtil will then identify and group all series of identical blocks, and
   * finally transform them into bars (in this terminology, a bar can cover several days as opposed
   * to a block).
   *
   ...
   * sorted so that identical blocks will appear in a consecutive order in that list => see order by
   * configuration,date in startLogic()
   */
  ...
  // Once the position of the bars are computed by barsLayout, they will be automatically drawn in a
  // canvas by this
  ...
  public AccommodationGantt(
    ...
    ThemeRegistry.runNowAndOnModeChange(
        () -> {
          parentRoomDrawer.setTextFont(
              barsFont = TextTheme.getFont(FontDef.font(FontWeight.BOLD, barsFontSize)));
    ...
    // Redrawing the canvas when Gantt selected object changes because the guest color may depend on
    // selected event
  ...
  public Node buildCanvasContainer() {
    // We embed everything in a scrollPane because the rooms probably won't all fit on the screen.
    // This scrollPane
    ...
    // That scrollPane will contain a splitPane showing the list of rooms on its left side, and the
    // blocks/bars on
    // the right side. To show the list of rooms, we just use a ParentCanvasPane which displays the
    // parents of the
    ...
    // resizes the UI, and
    // for calling the canvas refresher (the piece of code that redraws the canvas). TimeCanvasUtil
    // will actually
    // create that refresher using the passed barsLayout & barsDrawer.
    // 2) in addition, VirtualCanvasPane keeps the canvas size as small as possible when used in a
    // scrollPane to
    // prevent memory overflow. Whereas the virtual canvas represents the whole canvas that the user
    // seems to watch
    // and can have a very long height, the real canvas will be only the size of the scrollPane
    // viewport, and when
    // the user scrolls, VirtualCanvasPane is responsible for redrawing the canvas to the scrolled
    // position.
  ...
  protected void drawParentRoom(ResourceConfiguration rc, Bounds b, GraphicsContext gc) {
    // The only remaining property that needs to be set here is the room name that we display in the
    // bar middle
    parentRoomDrawer
  ...
  protected void showBeds() {
    barsLayout
        ...
        // WebFxKitLauncher.measureText(bar.getInstance().getPersonName(), barsFont).getWidth())
        .setHSpacing(2)
        .setVSpacing(2);
    parentsCanvasDrawer
