...
public abstract class AccommodationGantt<B extends AccommodationBlock> {
    ...
    /**
     * We will ask TimeBarUtil to automatically convert those Attendance entities first into
     * AttendanceBlock instances which are a reduction of those entities to the strictly minimal set
     * of fields required for the canvas draw, so here fields = resourceConfiguration (= parent from
     * gantt point of view) & available (the number displayed) and nothing more (we forget the
     * entityId and other fields). And because at this stage these instances will form blocks (one
     * instance per day), TimeBarUtil will then identify and group all series of identical blocks,
     * and finally transform them into bars (in this terminology, a bar can cover several days as
     * opposed to a block).
     *
     * <p>For example if room 208 has 2 beds available for 5 days, this series of 5 blocks (ie 5
     * Attendance instances with identical fields: resourceConfiguration = of 208 & available = 2)
     * will be grouped and transformed into a 5-days bar (holding 1 single instance of Attendance +
     * first & last day of that series).
     *
     * <p>Note that there are 2 conditions for this to work: 1) AttendanceBlock must implement
     * equals(), which is used by TimeBarUtil to identify identical blocks 2) The entities must be
     * sorted so that identical blocks will appear in a consecutive order in that list => see order
     * by configuration,date in startLogic()
     */
    ...
    protected final LocalDateGanttLayout<LocalDateBar<B>> barsLayout =
            new LocalDateGanttLayout<LocalDateBar<B>>()
                    ...
                    // .setChildGrandparentReader(bar ->
                    // bar.getInstance().getRoomConfiguration().getItem())
    ...
    // Once the position of the bars are computed by barsLayout, they will be automatically drawn in
    // a canvas by this
    ...
    protected final ParentsCanvasDrawer parentsCanvasDrawer =
            ParentsCanvasDrawer.create(barsLayout, barsDrawer)
    ...
    protected final BarDrawer barDrawer =
            new BarDrawer() // unique instance to draw all the bars
                    .setTextFill(Color.WHITE);

    protected final BarDrawer parentRoomDrawer =
            new BarDrawer() // unique instance to draw all the room names
    ...
    protected final BarDrawer bedDrawer =
            new BarDrawer()
    ...
    protected final BarDrawer grandparentRoomTypeDrawer =
            new BarDrawer() // unique instance to draw all the room types
                    .setStroke(Color.grayRgb(130))
                    .setBackgroundFill(Color.WHITE)
                    .setTextAlignment(TextAlignment.CENTER)
                    .setTextFill(Color.rgb(0, 150, 214));

    public AccommodationGantt(
            AccommodationPresentationModel pm,
            ObservableList<LocalDateBar<B>> children,
        ...
        // Pairing this Gantt canvas with the referent one (ie the event Gantt canvas on top), so it
        // always stays
        // horizontally aligned with the event Gantt dates, even when this canvas is horizontally
        // shifted (ex: when
        ...
        if (children != null) ObservableLists.bind(barsLayout.getChildren(), children);
        ...
        // Updating the text font on any theme mode change that may impact it (light/dark mode,
        // etc...)
        ThemeRegistry.runNowAndOnModeChange(
                () -> {
                    parentRoomDrawer.setTextFont(
                            barsFont =
                                    TextTheme.getFont(FontDef.font(FontWeight.BOLD, barsFontSize)));
        ...
        // Redrawing the canvas when Gantt selected object changes because the guest color may
        // depend on selected event
        FXProperties.runOnPropertiesChange(
                barsDrawer::markDrawAreaAsDirty, FXGanttSelection.ganttSelectedObjectProperty());
    }
    ...
    public Node buildCanvasContainer() {
        // We embed everything in a scrollPane because the rooms probably won't all fit on the
        // screen. This scrollPane
        // will be responsible for the vertical scrolling only, because the horizontal scrolling is
        // actually already
        // managed by the interactive canvas itself (which reacts to user dragging to move the gantt
        // dates).
        ScrollPane scrollPane = new ScrollPane();
        // That scrollPane will contain a splitPane showing the list of rooms on its left side, and
        // the blocks/bars on
        // the right side. To show the list of rooms, we just use a ParentCanvasPane which displays
        // the parents of the
        // barsLayout (the parents are ResourceConfiguration instances as set in
        // barsLayout.setChildParentReader() above)

        // We embed the canvas in a VirtualCanvasPane which has 2 functions:
        // 1) As a CanvasPane it is responsible for automatically resizing the canvas when the user
        // resizes the UI, and
        // for calling the canvas refresher (the piece of code that redraws the canvas).
        // TimeCanvasUtil will actually
        // create that refresher using the passed barsLayout & barsDrawer.
        // 2) in addition, VirtualCanvasPane keeps the canvas size as small as possible when used in
        // a scrollPane to
        // prevent memory overflow. Whereas the virtual canvas represents the whole canvas that the
        // user seems to watch
        // and can have a very long height, the real canvas will be only the size of the scrollPane
        // viewport, and when
        // the user scrolls, VirtualCanvasPane is responsible for redrawing the canvas to the
        // scrolled position.
        VirtualCanvasPane virtualCanvasPane =
                TimeCanvasUtil.createTimeVirtualCanvasPane(
                        barsLayout,
                        barsDrawer,
                        scrollPane.viewportBoundsProperty(),
                        scrollPane.vvalueProperty());
        // We finally set up the scrollPane for vertical scrolling only (no horizontal scrollbar,
        // etc...), and return it
    ...
    protected void drawGrandparentRoomType(Item item, Bounds b, GraphicsContext gc) {
        grandparentRoomTypeDrawer.setMiddleText(item.getName()).drawBar(b, gc);
    }

    protected void drawParentRoom(ResourceConfiguration rc, Bounds b, GraphicsContext gc) {
        // The only remaining property that needs to be set here is the room name that we display in
        // the bar middle
        parentRoomDrawer
        ...
        gc.fillRect(
                ...
                b.getMinY(),
                2,
                b.getHeight()); // erasing the left side of the stroke rectangle
    }
    ...
    protected void showBeds() {
        barsLayout
                ...
                // .setChildTetrisMinWidthReader(bar ->
                // WebFxKitLauncher.measureText(bar.getInstance().getPersonName(),
                // barsFont).getWidth())
                .setHSpacing(2)
                .setVSpacing(2);
        parentsCanvasDrawer
    ...
    protected void drawBed(Integer rowIndex, Bounds b, GraphicsContext gc) {
        bedDrawer.setMiddleText("Bed " + (rowIndex + 1)).drawBar(b, gc);
    }
}
