...
public final class ScheduledResourceLoader {
    ...
    // The results returned by the server will be stored in an observable list of ScheduledResource
    // entities:
    private final ObservableList<ScheduledResource> scheduledResources =
            FXCollections.observableArrayList();

    // Workaround for a WebFX push notification issue that happens when several identical reactive
    // entities mappers (ie
    // sending the exact same query and parameters to the server) run on the same client => the
    // issue is that the push
    // notifications are sent to only 1 instance at a time. The workaround is to keep a single
    ...
    public static ScheduledResourceLoader getOrCreate(AccommodationPresentationModel pm) {
        // Creating the instance on first call only (assuming the presentation model is identical on
        // subsequent calls)
        if (INSTANCE == null) INSTANCE = new ScheduledResourceLoader(pm);
        return INSTANCE;
    }
    ...
    public void startLogic(
            Object mixin) { // may be called several times with different mixins (due to workaround)
        ...
        if (mixin instanceof HasActiveProperty) {
            ...
            if (activeProperty == null) activeProperty = ap;
            else activeProperty = FXProperties.combine(activeProperty, ap, (a1, a2) -> a1 || a2);
        }
        if (rem == null) { // first call
            // This ReactiveEntitiesMapper will populate the provided parents of the GanttLayout
            // (indirectly from allScheduledResources observable list)
            rem =
                    ReactiveEntitiesMapper.<ScheduledResource>createPushReactiveChain(mixin)
                            ...
                            .always(
                                    orderBy(
                                            "configuration.item.ord,configuration.name,configuration,date")) // Order is important for TimeBarUtil (see comment on barsLayout)
                            // Returning events for the selected organization only (or returning an
                            // empty set if no organization is selected)
                            .ifNotNullOtherwiseEmpty(
                                    pm.organizationIdProperty(),
                                    o -> where("configuration.resource.site.organization=?", o))
                            // Restricting events to those appearing in the time window
                            .always(
                                    pm.timeWindowStartProperty(),
                                    startDate -> where("sr.date >= ?", startDate))
                            .always(
                                    pm.timeWindowEndProperty(),
                                    endDate -> where("sr.date <= ?", endDate))
        ...
        rem.bindActivePropertyTo(
                    activeProperty); // updating the reactive entities mapper active property
    }
}
