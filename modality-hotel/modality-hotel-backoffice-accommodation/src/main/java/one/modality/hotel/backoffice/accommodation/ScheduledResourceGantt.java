...
public abstract class ScheduledResourceGantt extends AccommodationGantt<ScheduledResourceBlock> {
  ...
  // The user has the option to enable/disable the blocks grouping (when disabled, TimeBarUtil will
  // not group the
    ...
    TimeBarUtil.convertToBlocksThenGroupToBars(
        ...
        ScheduledResourceBlock
            ::new, // the factory that creates blocks, initially 1 instance per entity, but then
                   // grouped into bars
        barsLayout
            .getChildren(), // the final list of bars that will receive the result of grouping
                            // blocks
        blocksGroupingProperty); // optional property to eventually disable the blocks grouping (=>
                                 // 1 bar per block if disabled)
