...
public final class ResourceConfigurationLoader {
    ...
    // The results returned by the server will be stored in an observable list of
    // ResourceConfiguration entities:
    private final ObservableList<ResourceConfiguration> resourceConfigurations =
    ...
    public static ResourceConfigurationLoader getOrCreate(AccommodationPresentationModel pm) {
        ...
        // subsequent calls)
        if (INSTANCE == null) INSTANCE = new ResourceConfigurationLoader(pm);
        return INSTANCE;
    }
        ...
        if (rem == null) { // first call
            ...
            rem =
                    ReactiveEntitiesMapper.<ResourceConfiguration>createPushReactiveChain(mixin)
                            .always(
                                    "{class: 'ResourceConfiguration', alias: 'rc', fields: 'name,item.name'}")
                            ...
                            .ifNotNullOtherwiseEmpty(
                                    ...
                                    o -> where("resource.site.(organization=? and event=null)", o))
