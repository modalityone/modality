...
public class HouseholdGanttCanvas {
    ...
    private static final double ROW_HEIGHT = 24;  // Row height reduced by 30% (was 34)
    private static final double BAR_HEIGHT = 34;  // Actual booking bar height (unchanged)
    private static final double BAR_RADIUS = 10;
    private static final double ROOM_HEADER_WIDTH = 130;  // Reduced by 35% (was 200)

    // Vertical margins to match original GridPane implementation (reduced by 30%)
    private static final double BOOKING_BAR_TOP_MARGIN = 5.6;  // Was 8, reduced by 30%
    private static final double BOOKING_BAR_BOTTOM_MARGIN = 5.6;  // Was 8, reduced by 30%
    ...
    protected final LocalDateGanttLayout<LocalDateBar<HouseholdBookingBlock>> barsLayout =
            new LocalDateGanttLayout<LocalDateBar<HouseholdBookingBlock>>()
                    ...
                    .setVSpacing(0.5)  // Vertical spacing between rows (reduced by 75% from 2)
     ...
     * - For room rows: Shows expand/collapse arrow (if multi-bed), status dot, room name
    ...
    protected void drawParentRoom(GanttParentRow parentRow, Bounds b, GraphicsContext gc) {
        ...
        if (parentRow.isBed()) {
            ...
            gc.fillText(parentRow.getDisplayName(), b.getMinX() + 8, b.getMinY() + b.getHeight() / 2);
        } else {
            // Draw room row
            parentRoomDrawer
                    .setBackgroundFill(Color.WHITE)
            ...
            double currentX = b.getMinX() + 8;  // Start position with padding
            ...
            if (parentRow.isMultiBedRoom()) {
                ...
                gc.setTextAlign(javafx.scene.text.TextAlignment.LEFT);
                gc.setTextBaseline(javafx.geometry.VPos.CENTER);
                gc.fillText(arrow, currentX, b.getMinY() + b.getHeight() / 2);

                currentX += 18;  // Move position past the arrow
            }
            ...
            if (room.getStatus() != null) {
                ...
                double dotRadius = 4;
                double dotX = currentX + dotRadius + 2;
                ...
                currentX = dotX + dotRadius + 6;  // Move position past the dot
            }
            ...
            gc.fillText(room.getName(), currentX, b.getMinY() + b.getHeight() / 2);

            // Draw room comments if present (small italic text below room name)
            if (room.getRoomComments() != null && !room.getRoomComments().isEmpty()) {
                gc.setFill(Color.web("#666666"));
                gc.setFont(javafx.scene.text.Font.font("System", javafx.scene.text.FontPosture.ITALIC, 11));
                gc.setTextBaseline(javafx.geometry.VPos.BOTTOM);
                gc.fillText(room.getRoomComments(), b.getMinX() + 8, b.getMinY() + b.getHeight() - 3);
            }
        }
    }
    ...
    protected void drawBar(LocalDateBar<HouseholdBookingBlock> bar, Bounds b, GraphicsContext gc) {
        ...
        // - Y: add top margin for vertical spacing
        // - Width: reduce by one day width (to end at middle of departure day)
        // - Height: reduce by top + bottom margins for vertical spacing
        ...
        adjustedBounds.setMinY(b.getMinY() + BOOKING_BAR_TOP_MARGIN);
        adjustedBounds.setWidth(b.getWidth() - dayWidth);
        adjustedBounds.setHeight(b.getHeight() - BOOKING_BAR_TOP_MARGIN - BOOKING_BAR_BOTTOM_MARGIN);
    ...
    private void drawRegularBookingBar(HouseholdBookingBlock block, Bounds adjustedBounds, Color barColor, GraphicsContext gc) {
        ...
        if (position == one.modality.hotel.backoffice.activities.household.gantt.model.BookingPosition.ARRIVAL ||
            ...
            // Apply 60% size to person icon (fits better in 34px bar)
            double personIconScaleFactor = 0.6;
            bookingBarDrawer.setIcon(
                HouseholdGanttIcons.PERSON_ICON_SVG_PATH,
                iconColor,
                HouseholdGanttIcons.PERSON_ICON_SVG_WIDTH * personIconScaleFactor,
                HouseholdGanttIcons.PERSON_ICON_SVG_HEIGHT * personIconScaleFactor,
            ...
            // Clear icon if none should be shown
