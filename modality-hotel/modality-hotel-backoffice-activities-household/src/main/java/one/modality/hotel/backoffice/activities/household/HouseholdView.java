...
final class HouseholdView {
    ...
    private static final Color OCCUPIED_COLOR = Color.rgb(130, 135, 136);
    private static final Color CLEANING_NEEDED_COLOR = Color.rgb(255, 3, 5);
    private static final Color CLEANED_DONE_COLOR = Color.rgb(65, 186, 77);
    private static final Color NEVER_OCCUPIED_COLOR = Color.ORANGE;
                ...
                { // We also override getBarColor() to show checked-in attendees as gray
                    {
                        ...
                        barsLayout
                                ...
                                .addListener(
                                        (observable, oldValue, bar) -> {
                                            if (bar != null) {
                                                AttendanceBlock block = bar.getInstance();
                                                if (block.isCheckedIn()) {
                                                    if (bar.getEndTime()
                                                            .isAfter(FXToday.getToday())) return;
                                                    ResourceConfiguration roomConfiguration =
                                                            block.getRoomConfiguration();
                                                    int bedIndex =
                                                            barsLayout.getRowIndexInParentRow(bar);
                                                    LocalDate lastCleaningDate =
                                                            roomConfiguration.getLastCleaningDate();
                                                    boolean needsCleaning =
                                                            (lastCleaningDate == null
                                                                    || lastCleaningDate.isBefore(
                                                                            bar.getEndTime()));
                                                    if (needsCleaning)
                                                        activity.executeOperation(
                                                                new MarkBedAsCleanedRequest(
                                                                        roomConfiguration,
                                                                        bedIndex,
                                                                        bar.getEndTime(),
                                                                        (Pane) activity.getNode()));
                                                }
                                                // Resetting the selection to null, so that the user
                                                // can select the same bar again
                                                barsLayout.setSelectedChild(null);
                                            }
                                        });
                        parentsCanvasDrawer.<ResourceConfiguration>setChildRowHeaderClickHandler(
                                (rc, bedIndex) -> {
                                    System.out.println(
                                            rc.getName()
                                                    + " - bed "
                                                    + (bedIndex + 1)
                                                    + " - lastCleaning: "
                                                    + rc.getLastCleaningDate());
                                    DocumentLine documentLine =
                                            findLatestOccupiedDocumentLineBeforeToday(rc, bedIndex);
                                    LocalDate checkInDate = getCheckInDate(rc, documentLine);
                                    LocalDate checkOutDate = getCheckOutDate(rc, documentLine);
                                    System.out.println(
                                            "[" + checkInDate + ", " + checkOutDate + "]");
                                    if (checkOutDate != null) {
                                        TimeWindowUtil.setTimeWindowCenter(
                                                pm,
                                                checkOutDate,
                                                barsLayout.getTimeProjector().getTemporalUnit());
                                    }
                                });
                    }
                    ...
                    protected Color getBarColor(LocalDateBar<AttendanceBlock> bar, Bounds b) {
                        ...
                        if (block.isCheckedIn()) {
                            ...
                            DocumentLine documentLine =
                                    bar.getInstance().getAttendance().getDocumentLine();
                            documentLine.setBedNumber(bedIndex);
                            if (documentLine
                                    == findLatestOccupiedDocumentLineBeforeToday(
                                            block.getRoomConfiguration(), bedIndex))
                                return getCleaningColor(
                                        block.getRoomConfiguration(),
                                        bedIndex,
                                        bar.getStartTime(),
                                        bar.getEndTime());
                    ...
                    protected void drawBed(Integer rowIndex, Bounds b, GraphicsContext gc) {
                        ...
                        // Trick: the passed Bounds is actually an ObjectBounds whose object is the
                        // parent, so here it's the room
                        // i.e. ResourceConfiguration.
                        ResourceConfiguration rc =
                                ((ObjectBounds<ResourceConfiguration>) b).getObject();
                        DocumentLine documentLine =
                                findLatestOccupiedDocumentLineBeforeToday(rc, rowIndex);
    ...
    private Color getCleaningColor(
            ResourceConfiguration rc, DocumentLine documentLine, int bedIndex) {
        LocalDate checkInDate = getCheckInDate(rc, documentLine);
        LocalDate checkOutDate = getCheckOutDate(rc, documentLine);
    ...
    private Color getCleaningColor(
        ...
        if (lastCheckInDate != null
                && lastCheckInDate.isBefore(today)
                && lastCheckOutDate != null
                && lastCheckOutDate.isAfter(today)) return OCCUPIED_COLOR;
        LocalDate lastCleaningDate = getLastBedCleaningDate(rc, bedIndex);
        if (lastCheckOutDate != null
                && (lastCleaningDate == null || lastCleaningDate.isBefore(lastCheckOutDate)))
            return CLEANING_NEEDED_COLOR;
        if (lastCleaningDate == null) return NEVER_OCCUPIED_COLOR;
        return CLEANED_DONE_COLOR;
    }

    private DocumentLine findLatestOccupiedDocumentLineBeforeToday(
            ResourceConfiguration rc, int bedIndex) {
        return attendances.stream()
                .filter(
                        a ->
                                a.getScheduledResource().getResourceConfiguration() == rc
                                        && !a.getDate().isAfter(FXToday.getToday()))
    ...
    // Note: in KBS2, a.scheduledResource.resourceConfiguration refers to the global site, while
    // dl.resourceConfiguration
    // refers to the event site.

    private LocalDate getCheckInDate(
            ResourceConfiguration rc, DocumentLine dl) { // rc = global site
        ...
        if (checkInDate == null) {
            checkInDate =
                    attendances.stream()
                            .filter(
                                    a ->
                                            a.getScheduledResource().getResourceConfiguration()
                                                            == rc
                                                    && a.getDocumentLine() == dl)
    ...
    void startLogic(Object mixin) {
        ...
        ReactiveEntitiesMapper.<Attendance>createPushReactiveChain(mixin)
                .always(
                        "{class: 'Attendance', alias: 'a', fields: 'date,documentLine.document.(arrived,person_firstName,person_lastName,event.id),scheduledResource.configuration.(name,item.name,lastCleaningDate),documentLine.document.event.name'}")
                // .always(where("scheduledResource.configuration.(lastCleaningDate == null or
                // lastCleaningDate < date)"))
                // Order is important for TimeBarUtil
                .always(
                        orderBy(
                                "scheduledResource.configuration.item.ord,scheduledResource.configuration.name,documentLine.document.person_lastName,documentLine.document.person_firstName,date"))
