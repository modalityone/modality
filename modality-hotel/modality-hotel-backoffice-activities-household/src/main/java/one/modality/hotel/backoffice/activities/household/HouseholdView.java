  ...
  HouseholdView(AccommodationPresentationModel pm, HouseholdActivity activity) {
    ...
    attendanceGantt =
        new AttendanceGantt(
            ...
            attendances, // attendances observable list that we provide as input to AttendanceGantt
                         // for bar conversion
        ...
        { // We also override getBarColor() to show checked-in attendees as gray
                    ...
                    (observable, oldValue, bar) -> {
                      if (bar != null) {
                        ...
                        if (block.isCheckedIn()) {
                          if (bar.getEndTime().isAfter(FXToday.getToday())) return;
                          ...
                          boolean needsCleaning =
                              (lastCleaningDate == null
                                  || lastCleaningDate.isBefore(bar.getEndTime()));
                          if (needsCleaning)
                        ...
                        // Resetting the selection to null, so that the user can select the same bar
                        // again
                ...
                (rc, bedIndex) -> {
                  ...
                  if (checkOutDate != null) {
                    TimeWindowUtil.setTimeWindowCenter(
                        pm, checkOutDate, barsLayout.getTimeProjector().getTemporalUnit());
                  }
                });
          }
            ...
            if (block.isCheckedIn()) {
              ...
              if (documentLine
                ...
                return getCleaningColor(
                    block.getRoomConfiguration(), bedIndex, bar.getStartTime(), bar.getEndTime());
            }
          ...
          protected void drawBed(Integer rowIndex, Bounds b, GraphicsContext gc) {
            ...
            // Trick: the passed Bounds is actually an ObjectBounds whose object is the parent, so
            // here it's the room
        ...
        .always(
            ...
            endDate ->
                where(
                    ...
                    endDate)) // -1 is to avoid the round corners on right for bookings exceeding
                              // the time window
