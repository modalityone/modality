 ...
 */
public interface HasFestivalDaySelectionSection extends BookingFormSection, ResettableSection {
     ...
     * When bound, the section will push date/time selection changes to the state.
     *
    ...
    default void bindToSelectionState(BookingSelectionState selectionState) {
        if (selectionState != null) {
            // Push date changes from section to state
            arrivalDateProperty().addListener((obs, oldVal, newVal) -> {
                selectionState.setArrivalDate(newVal);
            });
            departureDateProperty().addListener((obs, oldVal, newVal) -> {
                selectionState.setDepartureDate(newVal);
            });
            arrivalTimeProperty().addListener((obs, oldVal, newVal) -> {
                selectionState.setArrivalTime(newVal);
            });
            departureTimeProperty().addListener((obs, oldVal, newVal) -> {
                selectionState.setDepartureTime(newVal);
            });
            // Initialize state from current values
            if (getArrivalDate() != null) {
                selectionState.setArrivalDate(getArrivalDate());
            }
