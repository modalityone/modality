...
import one.modality.booking.frontoffice.bookingpage.sections.confirmation.DefaultTermsSection;
...
public class StandardBookingForm extends MultiPageBookingForm
    implements BookingFormPaymentHandler.PaymentFormCallback,
               BookingFormQueueHandler.QueueFormCallback,
               BookingFormSoldOutHandler.SoldOutFormCallback {
    ...
    // Selection state for user choices (implements Selection Model Pattern)
    private final BookingSelectionState selectionState = new BookingSelectionState();

    // Payment handler (extracted for maintainability)
    private final BookingFormPaymentHandler paymentHandler;

    // Queue handler (extracted for maintainability)
    private final BookingFormQueueHandler queueHandler;

    // Sold-out handler (extracted for maintainability)
    private final BookingFormSoldOutHandler soldOutHandler;
    ...
    private void showFailedPaymentContent(MoneyTransfer moneyTransfer) {
        ...
        // Set callbacks
        failedSection.setOnRetryPayment(() -> paymentHandler.handlePaymentSubmit(FXResumePayment.getAmount(), FXResumePayment.getPaymentAllocations()));
    ...
    private void wireUpInternalCallbacks() {
        ...
        if (defaultPaymentSection != null) {
            ...
            defaultPaymentSection.setOnPaymentSubmit(paymentHandler::handlePaymentSubmit);
        }
     ...
     */
    @Override
    public void updateSummaryPageButtons() {
        if (summaryPage instanceof CompositeBookingFormPage compositeSummary) {
            if (state.isReturnedFromSoldOutRecovery()) {
    ...
    private Future<?> handleSummaryContinueAsync() {
        ...
        state.setReturnedFromSoldOutRecovery(false);
        ...
        return callbacks.onSubmitBooking()
            ...
            .compose(submitResult -> {
                ...
                if (submitResult.status() == DocumentChangesStatus.ENQUEUED) {
                    queueHandler.handleEnqueued(submitResult, isNewUser, newUserName, newUserEmail, wantsAccountCreation);
    ...
    public void handleAccommodationSoldOut(SoldOutErrorParser.SoldOutInfo soldOutInfo) {
        // Delegate to the sold-out handler
        soldOutHandler.handleAccommodationSoldOut(soldOutInfo);
    }
     ...
     */
    @Override
    public void cancelBookingAndExit() {
        WorkingBooking workingBooking = getWorkingBooking();
        if (workingBooking != null) {
    ...
    /**
     * Gets the working booking.
     * Used by the sold-out handler to access document lines and price calculations.
     */
    @Override
    public WorkingBooking getWorkingBooking() {
        return workingBookingProperties != null ? workingBookingProperties.getWorkingBooking() : null;
    }

    /**
     * Sets the returned from sold-out recovery flag.
     * Called by the sold-out handler to track navigation state.
     */
    @Override
    ...
    // Note: Queue handling logic is now in BookingFormQueueHandler.
    // The methods below are callback implementations for the handler.
    ...
    public boolean isRegistrationOpen() {
        ...
        if (state.isRegistrationConfirmedOpen()) {
            return true;
        }
        PolicyAggregate policyAggregate = workingBookingProperties.getPolicyAggregate();
    ...
    /**
     * Sets the registration confirmed open flag.
     * Called by the queue handler when queue processing completes.
     */
    @Override
    public void setRegistrationConfirmedOpen(boolean confirmed) {
        state.setRegistrationConfirmedOpen(confirmed);
    }

    /**
     * Gets the working booking properties.
     */
    @Override
     ...
     */
    @Override
    public Future<Void> loadAllBookingsForEventAsync() {
            ...
            if (lines != null) {
                for (DocumentLine line : lines) {
                    ...
                    if (item != null) {
                        ...
                        String lineDates = BookingDateFormatter.computeDatesForDocumentLine(documentAggregate, line);
    ...
    private void handleRegisterAnotherPerson() {
        ...
        // Reset selection state (user choices)
    ...
    private void handleMakeAnotherBooking() {
        // Reset state (also clears storedNewUserName/storedNewUserEmail)
        state.reset();
        // Reset selection state (user choices)
        selectionState.reset();
    ...
    private void handleLogout() {
        ...
        // This also clears storedNewUserName/storedNewUserEmail
        state.prepareForNewBooking();
        // Reset selection state (user choices)
        selectionState.reset();

        // Reset the hadLoggedInUser flag since user is logging out
        state.setHadLoggedInUser(false);
     ...
     */
    @Override
    public void populatePendingBookingsForNewUser(String userName, String userEmail) {
        // Store new user info for confirmation section (state is cleared after submission)
        state.setStoredNewUserName(userName);
        state.setStoredNewUserEmail(userEmail);
    ...
    @Override
    public void updateConfirmationSection(StandardBookingFormCallbacks.PaymentResult result) {
        if (defaultConfirmationSection == null) return;
        ...
        if (hasBookingsFromPendingSection) {
            ...
            String storedName = state.getStoredNewUserName();
            if (storedName != null) {
                defaultConfirmationSection.addConfirmedBooking(new HasConfirmationSection.ConfirmedBooking(
                    storedName,
                    state.getStoredNewUserEmail(),
                    bookingRef));
            }
        }
    ...
    @Override
    public void navigateToSpecialPage(CompositeBookingFormPage page) {
        // Delegate to parent's protected method (widening visibility for PaymentFormCallback interface)
        super.navigateToSpecialPage(page);
    }
    ...
    /**
     * Returns the comments section (if enabled).
     * Can be used to bind to the selection state.
     *
     * @return the comments section, or null if not enabled
     */
    public DefaultCommentsSection getCommentsSection() {
        return defaultCommentsSection;
    }
}
