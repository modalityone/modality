 ...
 * @author Bruno Salmon
 */
public class OnlineTeachingSection implements BookingFormSection {
    ...
    public boolean isApplicableToBooking(WorkingBooking workingBooking) {
        return workingBooking.isNewBooking()
                || !workingBooking.getPolicyAggregate().getBookablePeriods(KnownItemFamily.TEACHING).isEmpty();
    }
    ...
    public void setWorkingBookingProperties(WorkingBookingProperties workingBookingProperties) {
        ...
        List<BookablePeriod> bookablePeriods = workingBooking.getPolicyAggregate()
                .getBookablePeriods(KnownItemFamily.TEACHING,
                        I18nKeys.upperCaseFirstChar(BookingFormI18nKeys.wholeEvent));
        ...
        // We display at the bottom an explanation of the VOD expiration date if the
        // event has one (ex: Festivals)
        LocalDateTime vodExpirationDate = workingBooking.getPolicyAggregate().getEvent().getVodExpirationDate();
        if (vodExpirationDate != null)
            I18nControls.bindI18nProperties(videoRecordingMessageLabel,
                    BookingPageI18nKeys.VideoRecordingExpirationMessage1,
                    LocalizedTime.formatLocalDateTimeProperty(vodExpirationDate,
                            FrontOfficeTimeFormats.MEDIA_EXPIRATION_DATE_TIME_FORMAT));
