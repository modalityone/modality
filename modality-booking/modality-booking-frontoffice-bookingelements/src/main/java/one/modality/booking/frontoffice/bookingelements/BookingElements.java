package one.modality.booking.frontoffice.bookingelements;
...
public final class BookingElements {
    ...
    private static EntityButtonSelector<Person> createPersonToBookSelector(boolean embedPersonToBookText, ButtonFactoryMixin buttonFactory, DataSourceModel dataSourceModel) {
        EntityButtonSelector<Person> personSelector = new EntityButtonSelector<Person>( // language=JSON5
            "{class: 'Person', alias: 'p', columns: [{expression: '[firstName,lastName]'}], fields: 'email,phone,street,postCode,cityName,country,organization, accountPerson', where: '!removed', orderBy: 'owner desc,id'}",
            ...
            .ifNotNullOtherwiseEmpty(FXModalityUserPrincipal.modalityUserPrincipalProperty(), mup -> DqlStatement.where("frontendAccount=$1", mup.getUserAccountId()))
