package mongoose.base.backoffice.activities.filters.routing;
...
public final class FiltersRouting {
    ...
    private final static String ANY_PATH = "/filters(/organization/:organizationId)?(/event/:eventId)?(/day/:day)?(/arrivals)?(/departures)?(/minday/:minDay)?(/maxday/:maxDay)?(/filter/:filter)?(/groupby/:groupBy)?(/orderby/:orderBy)?(/columns/:columns)?(/export/:activityStateId)?";
    private final static String EVENT_PATH = "/filters/event/:eventId";
