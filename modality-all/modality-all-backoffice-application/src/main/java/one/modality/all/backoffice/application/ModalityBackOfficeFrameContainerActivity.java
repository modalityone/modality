...
public class ModalityBackOfficeFrameContainerActivity extends ModalityClientFrameContainerActivity {
    ...
    public Node buildUi() {
        frameContainer =
                new Pane() {
                    ...
                    protected void layoutChildren() {
                        // TODO determine value to use for canvasPane based on which activity we are
                        // in
                        ...
                        if (ganttCanvasContainer.isVisible()) {
                            nodeHeight =
                                    ganttCanvasContainer.prefHeight(width)
                                            + breathingPadding.getTop()
                                            + (FXGanttInterstice.isGanttIntersticeRequired()
                                                    ? breathingPadding.getBottom()
                                                    : 0);
                            layoutInArea(
        ...
        FXProperties.runNowAndOnPropertiesChange(
                ...
                FXGanttInterstice.ganttIntersticeRequiredProperty(),
                frameContainer.widthProperty(),
                frameContainer.heightProperty());
        // When not in compact mode, the nodes don't cover the whole surface of this container,
        // because there are some
        // breathing areas (see breathingPadding) which appear as empty areas but with this
        // container background, so we
        // need to give these areas the same color as the nodes background (seen as primary facets
        // by the LuminanceTheme).
        LuminanceTheme.createPrimaryPanelFacet(frameContainer)
                .style(); // => will have the same background as the nodes
    ...
    private void updateMountNode() {
        // Note: the order of the children is important in compact mode, where the container header
        // overlaps the mount
        // node (as a transparent button bar on top of it) -> so the container header must be after
        // the mount node,
        // otherwise it will be hidden.
        frameContainer
                .getChildren()
                .setAll(
                        Collections.listOfRemoveNulls(
                                ganttCanvasContainer,
                                getMountNode(),
                                containerHeader,
                                containerFooter));
    ...
    protected HBox createContainerHeaderCenterItem() {
        ...
        EntityButtonSelector<Organization> organizationSelector =
        ...
        organizationSelector
                ...
                .bindBidirectional(FXOrganization.organizationProperty());
        Button organizationButton = organizationSelector.getButton();
        // Creating the event selector
        EntityButtonSelector<Event> eventSelector =
                new EntityButtonSelector<>(
                        "{class: 'Event'}", this, frameContainer, getDataSourceModel());
        // Doing a bidirectional binding with FXOrganization
        ...
        FXProperties.runNowAndOnPropertiesChange(
                () ->
                        eventSelector.setJsonOrClass(
                                "{class: 'Event', alias: 'e', columns: 'icon,name,dateIntervalFormat(startDate,endDate)', where: 'organization="
                                        + Entities.getPrimaryKey(
                                                organizationSelector.getSelectedItem())
        ...
        eventButton
                ...
                .bind(
                        FXProperties.compute(
                                ...
                                value -> value == GanttVisibility.EVENTS));
