...
public class ModalityBackOfficeFrameContainerActivity extends ModalityClientFrameContainerActivity {
  ...
  public Node buildUi() {
    ...
    // When not in compact mode, the nodes don't cover the whole surface of this container, because
    // there are some
    // breathing areas (see breathingPadding) which appear as empty areas but with this container
    // background, so we
    // need to give these areas the same color as the nodes background (seen as primary facets by
    // the LuminanceTheme).
  ...
  private void updateMountNode() {
    ...
    // overlaps the mount
    // node (as a transparent button bar on top of it) -> so the container header must be after the
    // mount node,
    ...
    frameContainer
        ...
        .setAll(
            Collections.listOfRemoveNulls(
                ganttCanvasContainer, getMountNode(), containerHeader, containerFooter));
  }
  ...
  protected HBox createContainerHeaderCenterItem() {
    ...
    EntityButtonSelector<Event> eventSelector =
        new EntityButtonSelector<>("{class: 'Event'}", this, frameContainer, getDataSourceModel());
    // Doing a bidirectional binding with FXOrganization
        ...
        () ->
            eventSelector.setJsonOrClass(
                "{class: 'Event', alias: 'e', columns: 'icon,name,dateIntervalFormat(startDate,endDate)', where: 'organization="
                    + Entities.getPrimaryKey(organizationSelector.getSelectedItem())
