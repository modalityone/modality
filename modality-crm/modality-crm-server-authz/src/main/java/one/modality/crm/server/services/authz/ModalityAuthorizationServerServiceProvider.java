...
public final class ModalityAuthorizationServerServiceProvider
    ...
    public Future<Void> pushAuthorizations() {
        // Capturing userId and runId from the thread local state holder (won't be present on
        // subsequent async callbacks)
                        ...
                        userClaims ->
                                QueryService.executeQuery(
                                                new QueryArgumentBuilder()
                                                        ...
                                                        .build())
                                        // Step 3: we push the query result to the Modality
                                        // authorization client counterpart
                                        .compose(
                                                queryResult ->
