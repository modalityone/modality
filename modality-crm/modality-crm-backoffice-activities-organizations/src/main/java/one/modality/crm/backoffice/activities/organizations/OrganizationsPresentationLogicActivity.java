...
final class OrganizationsPresentationLogicActivity
  ...
  private OrganizationsPresentationLogicActivity(
      Factory<OrganizationsPresentationModel> presentationModelFactory) {
  ...
  protected void startLogic(OrganizationsPresentationModel pm) {
    ReactiveVisualMapper.createPushReactiveChain(this)
        ...
        .ifTrue(
            pm.withEventsProperty(),
            "{where: 'exists(select Event where organization=o)', orderBy: 'id'}")
        // Limit condition
        ...
        // .setSelectedEntityHandler(organization -> new RouteToEventsRequest(organization,
        // getHistory()).execute())
        .start();
  }
}
