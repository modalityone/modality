...
public final class PersonalDetailsPanel implements ModalityButtonFactoryMixin {
  ...
  public PersonalDetailsPanel(Event event, ButtonFactoryMixin buttonFactoryMixin, Pane parent) {
    ...
    countrySelector =
        createEntityButtonSelector(
            "{class: 'Country', orderBy: 'name'}", buttonFactoryMixin, parent, dataSourceModel);
    ...
    FXProperties.runOnPropertiesChange(
        p -> syncUiFromModel((Person) p.getValue()), personSelector.selectedItemProperty());
    FXProperties.runNowAndOnPropertiesChange(
        userProperty -> {
          ...
          if (loggedIn) {
            ...
            personSelector.setJsonOrClass(
                "{class: 'Person', alias: 'p', fields: 'genderIcon,firstName,lastName,birthdate,email,phone,street,postCode,cityName,organization,country', columns: `[{expression: 'genderIcon,firstName,lastName'}]`, where: '!removed and frontendAccount="
                    + userAccountId
                    + "', orderBy: 'id'}");
            personSelector.autoSelectFirstEntity();
          } else personSelector.setJsonOrClass(null);
  ...
  private void initValidation() {
    ...
    validationSupport.addRequiredInput(
        maleRadioButton.getToggleGroup().selectedToggleProperty(), genderBox);
    validationSupport.addRequiredInputs(
        emailTextField, phoneTextField, carer1NameTextField, carer2NameTextField);
    validationSupport.addRequiredInput(
        countrySelector.selectedItemProperty(), countrySelector.getButton());
  }
  ...
  private static <T extends Entity> EntityButtonSelector<T> createEntityButtonSelector(
      Object jsonOrClass,
  ...
  private void updateUiEditable() {
    boolean profileEditable =
        editable && (personSelector == null || personSelector.getSelectedItem() == null);
  ...
  private GridPane createPersonGridPane() {
    GridPaneBuilder gridPaneBuilder =
        new GridPaneBuilder()
  ...
  private Node createPersonDataGrid() {
    ...
    VisualResultBuilder rsb =
        VisualResultBuilder.create(
            6, new VisualColumn[] {keyColumn, valueColumn, keyColumn, valueColumn});
    ...
    rsb.setValue(
        0, 3, organization == null ? I18n.getI18nText("NoCentre") : organization.getName());
    ...
    VisualGrid visualGrid =
        new VisualGrid(rsb.build()); // LayoutUtil.setMinMaxHeightToPref(new DataGrid(rsb.build()));
  ...
  public void syncUiFromModel(HasPersonalDetails p) {
    model = p;
    if (p instanceof Entity) setLoadingStore(((Entity) p).getStore());
    ...
    if (sectionPanel.getCenter() == null)
      FXProperties.runNowAndOnPropertiesChange(
          this::updatePanelBody, childRadioButton.selectedProperty(), I18n.dictionaryProperty());
    if (!editable) UiScheduler.runInUiThread(this::updatePanelBody);
  }
  ...
  private Integer computeAge(LocalDate birthDate) {
    Integer age = null;
    if (birthDate != null) {
      // Integer age = (int) birthDate.until(event.getStartDate(), ChronoUnit.YEARS); // Doesn't
      // compile with GWT
