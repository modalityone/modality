...
final class ModalityInMemoryUserAuthorizationChecker extends InMemoryUserAuthorizationChecker {
    ...
    private static final String AUTHZ_QUERY_BASE =
            "select rule.rule,activityState.route,operation.operationCode from AuthorizationAssignment";
    ...
    void onAuthorizationPush(Object pushObject) {
        ...
        QueryRowToEntityMapping queryMapping =
                dataSourceModel.parseAndCompileSelect(AUTHZ_QUERY_BASE).getQueryMapping();
        EntityStore entityStore = EntityStore.create(dataSourceModel);
        EntityList<Entity> assignments =
                QueryResultToEntitiesMapper.mapQueryResultToEntities(
                        queryResult, queryMapping, entityStore, "assignments");
        ruleRegistry.clear();
        for (Entity assignment : assignments) {
            ...
            // If it is a shared activity state, automatically granting the route to it (when
            // provided)
            ...
            if (activityState != null) {
                ...
                if (route != null) {
                    ...
                    ruleRegistry.registerAuthorizationRule(
                            new RoutingAuthorizationRule(
                                    AuthorizationRuleType.GRANT, route, false));
                }
            }
            Entity operation = assignment.getForeignEntity("operation");
            if (operation != null)
                ruleRegistry.registerAuthorizationRule(
                        "grant operation:" + operation.getStringFieldValue("operationCode"));
        }
