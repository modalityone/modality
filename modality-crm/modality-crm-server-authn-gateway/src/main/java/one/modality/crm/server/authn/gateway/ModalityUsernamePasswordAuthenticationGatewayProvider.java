...
public final class ModalityUsernamePasswordAuthenticationGatewayProvider
            ...
            QueryArgument.builder()
                ...
                .setParameters(
                    ...
                    usernamePasswordCredentials
                        .getPassword()) // "or true" is temporary to bypass the password checking
                                        // which is now encrypted TODO: implement encrypted version
                                        // of password checking
                .setDataSourceId(getDataSourceId())
                .build())
        .compose(
            result -> {
              if (result.getRowCount() != 1) return Future.failedFuture("Wrong user or password");
  ...
  public Future<UserClaims> getUserClaims() {
    return queryModalityUserInfo("frontendAccount.username,email,phone")
        .map(
            queryResult -> {
              // First 2 columns are person.id and frontendAccount.id, so we start collecting data
              // from column 2
  ...
  private Future<QueryResult> queryModalityUserInfo(String fields) {
    ...
    return QueryService.executeQuery(
            QueryArgument.builder()
                .setLanguage("DQL")
                .setStatement("select " + fields + " from Person where id=? and frontendAccount=?")
                .setParameters(
