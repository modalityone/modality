...
final class PaymentActivity extends CartBasedActivity {
  ...
  public Node buildUi() {
    ...
    BorderPane totalSection =
        SectionPanelFactory.createSectionPanelWithHeaderNodes(
            newLabel("TotalAmount:"), LayoutUtil.createHGrowable(), totalLabel = new Label());
    VBox vBox =
        new VBox(
            20, paymentPrompt, paymentsVBox, totalSection, newLargeGreenButton(makePaymentAction));
  ...
  private void displayDocumentPaymentsIfReady() {
    ...
    if (cartDocuments != null && paymentsVBox != null) {
      ...
      documentPayments =
          Collections.mapFilter(cartDocuments, DocumentPayment::new, DocumentPayment::hasBalance);
      UiScheduler.runInUiThread(
          () -> {
            paymentsVBox
                .getChildren()
                .setAll(Collections.map(documentPayments, DocumentPayment::getNode));
            updateTotal();
          });
    }
  }
  ...
  private final class DocumentPayment {
      ...
      if (node == null) {
        String title =
            document.getRef()
                + " - "
                + document
                    .getFullName(); // + " - " + I18n.instantTranslate("Booking") + " " +
                                    // document.getRef() + "   " + I18n.instantTranslate("Fee:") + "
        ...
        amountTextField
            .focusedProperty()
            .addListener((observable, oldValue, focused) -> updateAmount(amount, false));
        slider = new Slider(minAmount, maxAmount, maxAmount);
        slider
            .valueProperty()
            .addListener(
                (observable, oldValue, newValue) -> updateAmount(newValue.intValue(), true));
        if (notPaidFullCount > 1) zeroRadioButton = addRadioButton(0, "notNow");
        if (minAmount > 0 && minAmount < maxAmount)
          minRadioButton =
              addRadioButton(
                  minAmount,
                  document.getPriceDeposit() < document.getPriceMinDeposit() ? "minDeposit" : null);
        addNode(slider);
    ...
    private void updateAmount(int amount, boolean updateTotal) {
      ...
      radioGroup.selectToggle(
          amount == maxAmount
              ? maxRadioButton
              : amount == minAmount ? minRadioButton : amount == 0 ? zeroRadioButton : null);
      updating = false;
  ...
  private void submitPayment() {
    ...
    if (payments.size() == 1) {
      ...
      for (DocumentPayment payment : payments) {
        ...
        paymentRef =
            (paymentRef == null ? "" : paymentRef + "-") + payment.document.getRef().toString();
      }
    }
    updateStore
        .submitChanges()
                      ...
                      entityLists -> {
                        ...
                        UiScheduler.runInUiThread(
                            () -> {
                              ...
                              HtmlText htmlText =
                                  LayoutUtil.setMaxPrefSizeToInfinite(new HtmlText(innerHtml));
                              DialogUtil.showModalNodeInGoldLayout(
                                  htmlText, (Pane) getNode(), 0.9, 0.8);
                            });
                      });
            });
  }
  ...
  private String generateHtmlForm(List<GatewayParameter> gatewayParameters) {
    ...
    gatewayParameters =
        Collections.filter(gatewayParameters, gp -> live && gp.isLive() || test && gp.isTest());
    ...
    if (certificate != null) { // certificate => computing vads signature
      ...
      for (GatewayParameter gp : gatewayParameters) {
        if (gp.getName().startsWith("vads_")) signature += replaceBrackets(gp.getValue()) + '+';
      }
  ...
  private String replaceBrackets(String value) {
    ...
    value =
        Strings.replaceAllSafe(
            value,
            "[country]",
            doc.getCountry() != null ? doc.getCountry().getName() : doc.getCountryName());
    value =
        Strings.replaceAllSafe(
            value,
            "[countryCode]",
    ...
    value =
        Strings.replaceAllSafe(
            value, "[paymentId6]", digits(lastPayment.getPrimaryKey().toString(), 6, false));
    value =
        Strings.replaceAllSafe(
