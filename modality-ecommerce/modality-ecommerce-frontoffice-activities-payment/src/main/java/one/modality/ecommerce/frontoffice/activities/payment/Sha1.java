 ...
 * Version 2.1a Copyright Paul Johnston 2000 - 2002. Other contributors: Greg Holt, Andrew Kepert,
 * Ydnar, Lostinet Distributed under the BSD License See http://pajhome.org.uk/crypt/md5 for
 * details.
 */
...
final class Sha1 {
  ...
  static String hash(String msg) {
    try {
      byte[] bytes =
          msg.getBytes(
              StandardCharsets
                  .UTF_8); // convert string to UTF-8, as SHA only deals with byte-streams
  ...
  private static String hash(byte[] bytes) {
    ...
    double l =
        ((double) length) / 4 + 2; // length (in 32-bit integers) of msg + ‘1’ + appended length
