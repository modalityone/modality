package one.modality.ecommerce.client.controls.bookingoptionspanel;
...
public final class BookingOptionsPanel {
    ...
    public BookingOptionsPanel() {
        ...
        new AggregateFunction<DocumentLine>("days_agg", PrimType.STRING) {
            @Override
            public Object evaluateOnAggregates(
                    DocumentLine referrer,
                    Object[] aggregates,
                    Expression<DocumentLine> operand,
                    DomainReader<DocumentLine> domainReader) {
                DaysArrayBuilder daysArrayBuilder = new DaysArrayBuilder();
                for (Object dl : aggregates) {
                    DaysArray daysArray =
                            (DaysArray) ((DocumentLine) dl).getFieldValue("daysArray");
    ...
    public void syncUiFromModel(WorkingDocument workingDocument) {
        registerFormatter(
                "priceWithCurrency",
                new EventPriceFormatter(workingDocument.getEventAggregate().getEvent()));
        workingDocument.getComputedPrice(); // ensuring the price has been computed
        // Doesn't work on Android:
        // syncUiFromModel(workingDocument.getWorkingDocumentLines().stream().map(BookingOptionsPanel::createDocumentLine).filter(Objects::nonNull).collect(Collectors.toList()), workingDocument.getDocument().getStore());
        syncUiFromModel(
                Collections.mapFilter(
                        workingDocument.getWorkingDocumentLines(),
                        BookingOptionsPanel::createDocumentLine,
                        Objects::nonNull),
                workingDocument.getDocument().getStore());
    }
    ...
    private VisualResult generateDetailedLinesResult() {
        return EntitiesToVisualResultMapper.selectAndMapEntitiesToVisualResult(
                lineEntities,
                "select ["
                        + "'item.icon',"
                        + "'translate(item)',"
    ...
    private VisualResult generateGroupedLinesResult() {
        return EntitiesToVisualResultMapper.selectAndMapEntitiesToVisualResult(
                ...
                "select ["
                        +
                        // Displaying the actual item if only one is present for the item family,
                        // otherwise just displaying the item family (without further details)
