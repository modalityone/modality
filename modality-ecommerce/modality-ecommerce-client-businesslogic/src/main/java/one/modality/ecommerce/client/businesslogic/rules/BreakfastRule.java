...
public final class BreakfastRule extends BusinessRule {
        ...
        if (!wd.hasAccommodation() || !wd.hasMeals()) wd.removeBreakfast();
            ...
            if (breakfastOption != null) {
                ...
                List<WorkingDocumentLine>
                        breakfastLines =
                                new ArrayList<>(
                                        wd.getBusinessLines(BusinessType.BREAKFAST)
                                                .getBusinessWorkingDocumentLines()),
                        lunchLines =
                                wd.getBusinessLines(BusinessType.LUNCH)
                                        .getBusinessWorkingDocumentLines(),
                        supperLines =
                                wd.getBusinessLines(BusinessType.SUPPER)
                                        .getBusinessWorkingDocumentLines(),
                        accommodationLines =
                                wd.getBusinessLines(BusinessType.ACCOMMODATION)
                                        .getBusinessWorkingDocumentLines();
                // Iterating over each accommodation line to add or update the associated breakfast
                // line
    ...
    private static void addOrUpdateBreakfastLine(
        ...
        DaysArray accommodationDays =
                accommodationLine.getDaysArray().changeTimeUnit(TimeUnit.DAYS);
        ...
        if (Collections.noneMatch(
                        supperLines, wdl -> wdl.getDaysArray().overlaps(accommodationDays))
                && Collections.noneMatch(
                        lunchLines, wdl -> wdl.getDaysArray().overlaps(breakfastDays))) return;
        // All checks passed, so we need to add breakfast!
        // Trying to update an existing breakfast line (it must be the one overlapping that period
        // if several)
    ...
    private static Option getBreakfastOption(EventAggregate eventAggregate) {
        ...
        if (breakfastOption == null)
            eventAggregate.setBreakfastOption(
                    breakfastOption =
                            eventAggregate.findFirstConcreteOption(OptionLogic::isBreakfastOption));
