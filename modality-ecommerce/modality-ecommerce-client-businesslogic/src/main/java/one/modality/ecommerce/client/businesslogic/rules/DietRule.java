...
public final class DietRule extends BusinessRule {
  ...
  private static Option getDefaultDietOption(EventAggregate eventAggregate) {
    ...
    // If meals are included by default, then we return a default diet option (the first proposed
    // one) which will be
    ...
    // If meals are not included by default, we don't return a default diet option so bookers will
    // need to
