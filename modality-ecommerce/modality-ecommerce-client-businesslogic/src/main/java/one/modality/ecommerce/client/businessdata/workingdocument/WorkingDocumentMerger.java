...
public final class WorkingDocumentMerger {
  ...
  public static WorkingDocument mergeWorkingDocuments(
      WorkingDocument wd, WorkingDocument calendarWorkingDocument, DateTimeRange dateTimeRange) {
    List<WorkingDocumentLine> lines =
        new ArrayList<>(calendarWorkingDocument.getWorkingDocumentLines());
    // If the calendar working document (coming from the preselected options) has accommodation but
    // not the current
      ...
      if (line == null) {
        ...
        if (line.isAccommodation()) {
          ...
          lines.removeIf(
              wdl ->
                  wdl.isAccommodation()
                      && wdl.getDateTimeRange().overlaps(finalLine.getDateTimeRange()));
        }
        lines.add(line);
      }
