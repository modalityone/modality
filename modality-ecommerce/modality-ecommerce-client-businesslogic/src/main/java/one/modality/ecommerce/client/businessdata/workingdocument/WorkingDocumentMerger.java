...
public final class WorkingDocumentMerger {
    ...
    public static WorkingDocument mergeWorkingDocuments(
            WorkingDocument wd,
            WorkingDocument calendarWorkingDocument,
            DateTimeRange dateTimeRange) {
        List<WorkingDocumentLine> lines =
                new ArrayList<>(calendarWorkingDocument.getWorkingDocumentLines());
        // If the calendar working document (coming from the preselected options) has accommodation
        // but not the current
        // working document (because the booker probably unselected it), we should not reestablish
        // it
        if (!wd.hasAccommodation()) lines.removeIf(WorkingDocumentLine::isAccommodation);
        // Same thing with meals
        if (!wd.hasMeals()) lines.removeIf(WorkingDocumentLine::isMeals);
        for (WorkingDocumentLine thisLine : wd.getWorkingDocumentLines()) {
            WorkingDocumentLine line =
                    calendarWorkingDocument.findSameWorkingDocumentLine(thisLine);
            if (line == null) {
                ...
                if (line.isAccommodation()) {
                    ...
                    lines.removeIf(
                            wdl ->
                                    wdl.isAccommodation()
                                            && wdl.getDateTimeRange()
                                                    .overlaps(finalLine.getDateTimeRange()));
                }
                lines.add(line);
            }
