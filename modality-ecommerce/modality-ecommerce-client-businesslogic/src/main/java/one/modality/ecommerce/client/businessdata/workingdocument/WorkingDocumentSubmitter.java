...
public final class WorkingDocumentSubmitter {
  ...
  public static Future<Document> submit(WorkingDocument wd, String comment) {
    ...
    Document du;
    if (loadedWorkingDocument != null) du = store.updateEntity(loadedWorkingDocument.getDocument());
    else {
  ...
  private static void removeLine(WorkingDocument wd, DocumentLine dl) {
    wd.getUpdateStore()
        .deleteEntity(
            dl); // TODO: should probably be cancelled instead in some cases (and keep the non
                 // refundable part)
  }
}
