...
public final class HotelShuttleRule extends BusinessRule {
  ...
  public void apply(WorkingDocument wd) {
    ...
    if (!wd.hasAccommodation()) wd.removeHotelShuttle();
    else {
      List<Option> selectedHotelShuttleOptions =
          wd.getIsOptionSelectedInOptionTreePredicate() == null
              ? null
              : // null if the working document is not from the options activity
              wd.getEventAggregate()
                  .selectOptions(
                      o ->
                          OptionLogic.isHotelShuttleOption(o)
                              && wd.getIsOptionSelectedInOptionTreePredicate().test(o));
      // Fetching the existing hotel shuttle
      List<WorkingDocumentLine> hotelShuttleLines =
          wd.getBusinessLines(BusinessType.HOTEL_SHUTTLE).getBusinessWorkingDocumentLines();
      ...
      if (!Collections.isEmpty(selectedHotelShuttleOptions) || !hotelShuttleLines.isEmpty()) {
        hotelShuttleLines =
            new ArrayList<>(
                hotelShuttleLines); // Duplicating the list because updated shuttle lines will be
                                    // removed from that list
        // Iterating over each accommodation line to add or update the associated hotel shuttle
        // lines
        for (WorkingDocumentLine accommodationLine :
            wd.getBusinessLines(BusinessType.ACCOMMODATION).getBusinessWorkingDocumentLines())
          addOrUpdateHotelShuttleLines(
              accommodationLine, hotelShuttleLines, selectedHotelShuttleOptions);
      }
  ...
  private static void addOrUpdateHotelShuttleLines(
      WorkingDocumentLine accommodationLine,
      List<WorkingDocumentLine> hotelShuttleLines,
      List<Option> selectedHotelShuttleOptions) {
    // First check: accommodation line must be a hotel
    Site hotel = accommodationLine.getSite();
    if (!OptionLogic.isHotel(hotel)) return;
    ...
    if (accommodationDays.isEmpty()) return;
    ...
    for (long shiftDays = 0; shiftDays <= 1; shiftDays++) {
      ...
      // Second check: hotel shuttle is not added if there is no such inward/return selected option
      // for this hotel
      Option selectedHotelShuttleOption;
      if (selectedHotelShuttleOptions != null) {
        selectedHotelShuttleOption =
            Collections.findFirst(
                selectedHotelShuttleOptions,
                o -> (inward ? o.getArrivalSite() : o.getSite()) == hotel);
        if (selectedHotelShuttleOption == null) return;
      } else selectedHotelShuttleOption = null;
      DaysArray hotelShuttleDays =
          accommodationDays.shift(
              shiftDays); // same attendance as accommodation for inward and +1 day for return
                          // shuttle
      // All checks passed, so we need to add hotel shuttle!
      // Trying to update an existing hotel shuttle line (it must be the one overlapping that period
      // if several)
      WorkingDocumentLine hotelShuttleLine =
          Collections.findFirst(
              hotelShuttleLines,
              wdl ->
                  (selectedHotelShuttleOption != null
                          ? wdl.getOption() == selectedHotelShuttleOption
                          : (inward ? wdl.getArrivalSite() : wdl.getSite()) == hotel)
                      && wdl.getDaysArray()
                          .changeTimeUnit(TimeUnit.DAYS)
                          .overlaps(hotelShuttleDays));
      if (hotelShuttleLine != null) { // If found, setting the days to apply
        ...
        hotelShuttleLines.remove(
            hotelShuttleLine); // Also removing it from the list to not reuse it for a second update
