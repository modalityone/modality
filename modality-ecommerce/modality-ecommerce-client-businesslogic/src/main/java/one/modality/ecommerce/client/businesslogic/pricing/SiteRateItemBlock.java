...
final class SiteRateItemBlock {
    ...
    if (!rates.isEmpty()) {
      ...
      while (remainingDays > 0) {
        ...
        for (Rate rate : rates) {
          ...
          int quantity =
              1; // rate.perPerson && dl.share_owner && dl.capacity ? dl.capacity.capacity : 1;
          ...
          // When a rate defines a new lower daily price that applies after a minimum of days (ex:
          // 30% discount when >= 14 days),
          // we need to ensure that people approaching that number of days (ex: 12 or 13 days)
          // don't pay more with the previous rate than people staying that minimum of days (ex: 14
          // days)
          // In other words, we need to put an upper limit for such people, equals to the price that
          // is applied at that minimum of days
          if (blockLength < minDay
              && maxDay == 1) { // So if the block is less than the rate min day,
            ratePrice =
                ratePrice
                    * minDay; // we transform the daily rate into a fixed rate with the upper limit
            maxDay = minDay; // that applies over that period
          }
          if (startDate == null
              && endDate == null
              && blockLength <= maxDay
              && (cheapestFixedThreshold == null || cheapestFixedThreshold.price > ratePrice))
  ...
  public String toString() {
    StringBuilder sb =
        new StringBuilder("SiteRateItemBlock[" + site.getId() + " - " + rateItem.getId() + " -");
