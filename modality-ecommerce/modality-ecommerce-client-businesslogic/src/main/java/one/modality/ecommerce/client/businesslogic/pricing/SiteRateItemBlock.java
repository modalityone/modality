...
final class SiteRateItemBlock {
    ...
    int computePrice() {
        ...
        if (remainingDays == 0) return price;
        int consumedDays = 0;
        // if (update)
        ...
        List<Rate> rates =
                workingDocument
                        ...
                        .selectRates(
                                r ->
                                        Entities.sameId(r.getSite(), site)
                                                && Entities.sameId(r.getItem(), rateItem)
                                                && rateMatchesDocument(r)
                                                && dateNullOrBefore(r.getStartDate(), lastDay)
                                                && dateNullOrAfter(r.getEndDate(), firstDay));
        if (!rates.isEmpty()) {
            // rates.sort(function (r1, r2) { return (r1.perDay ? 1 : r1.maxDay ) - (r2.perDay ? 1 :
            // r2.maxDay);});
            while (remainingDays > 0) {
                // selecting the cheapest rate for the next attendances
                RateInfo cheapest = null;
                RateInfo second = null;
                RateInfo cheapestFixedThreshold =
                        null; // Used for Festivals while daily rates are variable
                for (Rate rate : rates) {
                    ...
                    // When a rate defines a new lower daily price that applies after a minimum of
                    // days (ex: 30% discount when >= 14 days),
                    // we need to ensure that people approaching that number of days (ex: 12 or 13
                    // days)
                    // don't pay more with the previous rate than people staying that minimum of
                    // days (ex: 14 days)
                    // In other words, we need to put an upper limit for such people, equals to the
                    // price that is applied at that minimum of days
                    ...
                    if (startDate == null
                            && endDate == null
                            && blockLength <= maxDay
                            && (cheapestFixedThreshold == null
                                    || cheapestFixedThreshold.price > ratePrice))
                        cheapestFixedThreshold =
                                new RateInfo(ratePrice / blockLength, ratePrice, blockLength);
                    ...
                    if (cheapest == null) cheapest = memo;
                    else if (dailyPrice < cheapest.dailyPrice) {
                        second = cheapest;
                        cheapest = memo;
                    } else if (second == null || dailyPrice < second.dailyPrice) second = memo;
                }
                if (cheapest == null) break;
                if (cheapestFixedThreshold != null
                        && price + cheapest.price > cheapestFixedThreshold.price) {
                ...
                // applying the found cheapest rate on the next consumable days (applicable for this
                // rate)
                int remainingPrice = cheapest.price;
                if (second == null) second = cheapest;
                ...
                for (int i = 0; i < cheapest.consumableDays && remainingPrice > 0; i++) {
                    ...
                    ba.setPrice(
                            i == cheapest.consumableDays - 1
                                    ? remainingPrice
                                    : Math.min(fullAttendancePrice, remainingPrice));
                    // if (update)
    ...
    private Integer getRatePrice(Rate rate) {
        ...
        if (age != null) {
            ...
            if (ageMax != null && age <= ageMax) {
                ...
                if (discount != null) return ratePrice * (100 - discount) / 100;
            }
    ...
    public String toString() {
        StringBuilder sb =
                new StringBuilder(
                        "SiteRateItemBlock[" + site.getId() + " - " + rateItem.getId() + " -");
