...
public final class WorkingDocumentTransaction {
    ...
    public WorkingDocumentLine addOption(Option option) {
        // Note: getInitialWorkingDocumentDateTimeRange() must be called before to capture the
        // initial date time range if not yet done
        WorkingDocumentLine line =
                new WorkingDocumentLine(
                        option, workingDocument, getInitialWorkingDocumentDateTimeRange());
    ...
    public void commit() {
        // Calling business rules. As there are possible changes that the working document is not
        // aware of (such as
        ...
        if (initialWorkingDocumentDateTimeRange != null) {
            ...
            if (!workingDocumentInterval.equals(
                    initialWorkingDocumentDateTimeRange.getInterval())) {
                // If yes, we should reinitialize the empty lines because they may now be included
                // in the new interval
                // (Ex: adding an arrival airport shuttle option may cause the early lunch be
                // attended by default)
