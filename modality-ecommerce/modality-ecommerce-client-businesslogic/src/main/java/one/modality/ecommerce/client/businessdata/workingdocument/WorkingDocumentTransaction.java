...
/**
 * A transaction for a working document to be used for a very short time such as reacting to a click
 * in the booking form.
 *
 * @author Bruno Salmon
 */
public final class WorkingDocumentTransaction {
  ...
  private DateTimeRange getInitialWorkingDocumentDateTimeRange() {
    if (initialWorkingDocumentDateTimeRange == null)
      initialWorkingDocumentDateTimeRange =
          new DateTimeRange(workingDocument.getDateTimeRange().getInterval());
    return initialWorkingDocumentDateTimeRange;
  }

  public WorkingDocumentLine addOption(Option option) {
    // Note: getInitialWorkingDocumentDateTimeRange() must be called before to capture the initial
    // date time range if not yet done
    WorkingDocumentLine line =
        new WorkingDocumentLine(option, workingDocument, getInitialWorkingDocumentDateTimeRange());
  ...
  public void removeOption(Option option) {
    ...
    Collections.removeIf(
        workingDocument.getWorkingDocumentLines(),
        wdl -> isOptionBookedInWorkingDocumentLine(wdl, option));
  }
  ...
  private static boolean isOptionBookedInWorkingDocumentLine(
      WorkingDocumentLine wdl, Option option) {
    Option wdlOption = wdl.getOption();
    return wdlOption != null
        ? wdlOption == option
        : wdl.getSite() == option.getSite() && wdl.getItem() == option.getItem();
  }

  public void commit() {
    // Calling business rules. As there are possible changes that the working document is not aware
    // of (such as
    // a selected option button but with no new lines added yet - they will be added thanks to
    // business rules),
    workingDocument
        .markAsChangedForBusinessRules(); // we call this method to force business rules execution.
    ...
    if (initialWorkingDocumentDateTimeRange != null) {
      ...
      if (!workingDocumentInterval.equals(initialWorkingDocumentDateTimeRange.getInterval())) {
        // If yes, we should reinitialize the empty lines because they may now be included in the
        // new interval
        // (Ex: adding an arrival airport shuttle option may cause the early lunch be attended by
        // default)
      ...
      initialWorkingDocumentDateTimeRange =
          null; // For next possible reuse of this transaction instance
    }
  }
}
