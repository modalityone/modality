...
final class StatementsActivity extends EventDependentViewDomainActivity
  ...
  public Node buildUi() {
    ...
    EntityButtonSelector<Entity> moneyAccountSelector =
        new EntityButtonSelector<>(
            ...
            // .combineIfNotNullOtherwiseForceEmptyResult(pm.organizationIdProperty(),
            // organizationId -> where("organization=?", organizationId))
            .ifNotNullOtherwiseEmpty(
                pm.eventIdProperty(),
                eventId ->
                    where(
                        "(event=? or event=null) and organization=(select organization from Event where id=?)",
                        eventId,
                        eventId));
  ...
  protected void startLogic() {
    ...
    groupVisualMapper =
        ReactiveVisualMapper.<MoneyTransfer>createGroupReactiveChain(this, pm)
            ...
            .ifNotNullOtherwiseEmpty(
                pm.selectedMoneyAccountProperty(),
                ma ->
                    where(
                        "parent = null and (fromMoneyAccount=? or toMoneyAccount=?) or parent != null and (parent..fromMoneyAccount=? or parent..toMoneyAccount=?",
                        ma,
                        ma,
                        ma,
                        ma))
            ...
            .ifFalse(
                pm.flatBatchesProperty(),
                where("transfer=null and (parent=null || parent..transfer=null)"))
            .start();
            ...
            .ifTrimNotEmpty(
                ...
                s ->
                    Character.isDigit(s.charAt(0))
                        ...
                        : s.contains("@")
                            ? where(
                                "lower(document.person_email) like ?", "%" + s.toLowerCase() + "%")
                            : DqlStatement.where(
                                "document.person_abcNames like ?", AbcNames.evaluate(s, true)))
    ...
    slaveVisualMapper =
        ReactiveVisualMapper.<MoneyTransfer>createSlavePushReactiveChain(this, pm)
            ...
            .ifNotNullOtherwiseEmpty(
                pm.selectedPaymentProperty(),
                mt -> where("parent=? or transfer=? or parent..transfer=?", mt, mt, mt))
            // Applying the flat modes
            .ifFalse(pm.flatPaymentsProperty(), where("parent=null"))
            .applyDomainModelRowStyle() // Colorizing the rows
            .start();
