...
final class DeleteMoneyAccountExecutor {
    ...
    private static String buildDeleteMoneyAccountMsg(
            MoneyAccount moneyAccount, List<MoneyFlow> moneyFlows) {
        List<MoneyFlow> moneyFlowsLinkedToAccount =
                getMoneyFlowsLinkedToAccount(moneyAccount, moneyFlows);

        if (moneyFlowsLinkedToAccount.isEmpty()) {
            return "Are you sure you wish to delete " + moneyAccount.getName() + "?";
        } else {
            String joinedAccountNames =
                    moneyFlowsLinkedToAccount.stream()
                            .map(
                                    moneyFlow ->
                                            moneyFlow.getToMoneyAccount().equals(moneyAccount)
                                                    ? moneyFlow.getFromMoneyAccount().getName()
                                                    : moneyFlow.getToMoneyAccount().getName())
                            .sorted(String::compareToIgnoreCase)
                            .collect(Collectors.joining("\n"));

            return moneyAccount.getName()
                    + " has money flows with the following accounts:\n\n"
                    + joinedAccountNames
                    + "\n\nThese money flows will also be deleted. Continue?";
        }
    }

    private static List<MoneyFlow> getMoneyFlowsLinkedToAccount(
            MoneyAccount moneyAccount, List<MoneyFlow> moneyFlows) {
    ...
    private static void deleteSelectedMoneyAccount(
            MoneyAccount moneyAccount, List<MoneyFlow> moneyFlows) {
        List<MoneyFlow> moneyFlowsLinkedToAccount =
                getMoneyFlowsLinkedToAccount(moneyAccount, moneyFlows);
