...
final class ContactUsActivity extends ViewDomainActivityBase implements ModalityButtonFactoryMixin {
  ...
  private final Action sendAction =
      newAction(
          "Send", "{url: 'images/svg/mono/send-circle.svg', width: 32, height: 32}", this::send);
  ...
  private void send() {
    ...
    mail.setOut(
        false); // indicate that this mail is not an outgoing email (sent to booker) but an ingoing
                // mail (sent to registration team)
