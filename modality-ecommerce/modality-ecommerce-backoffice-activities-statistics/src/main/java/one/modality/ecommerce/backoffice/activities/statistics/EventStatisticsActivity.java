...
final class EventStatisticsActivity extends EventDependentViewDomainActivity
  ...
  @Override
  public Node buildUi() {
    ConventionalUiBuilder ui =
        createAndBindGroupMasterSlaveViewWithFilterSearchBar(pm, "statistics", "DocumentLine");
    ...
    setUpContextMenu(
        ...
        () ->
            newActionGroup(
                newOperationAction(
                    () -> new SendLetterRequest(pm.getSelectedDocument(), container)),
                newSeparatorActionGroup(
                    newOperationAction(
                        () -> new EditDocumentLineRequest(pm.getSelectedDocumentLine(), container)),
                    newOperationAction(
                        () ->
                            new ToggleCancelDocumentLineRequest(
                                pm.getSelectedDocumentLine(), container)),
                    newOperationAction(
                        () ->
                    ...
                    newOperationAction(
                        () ->
                            new CopySelectionRequest(
                                ...
                                masterVisualMapper.getEntityColumns())),
                    newOperationAction(
                        () ->
                            new CopyAllRequest(
                                masterVisualMapper.getCurrentEntities(),
                                masterVisualMapper.getEntityColumns())))));
  ...
  protected void startLogic() {
    ...
    rightAttendanceVisualMapper =
        ReactiveVisualMapper.<Attendance>createReactiveChain(this)
            ...
            .ifNotNullOtherwiseEmpty(
                pm.conditionDqlStatementProperty(),
                conditionDqlStatement -> {
                  DqlClause where = conditionDqlStatement.getWhere();
                  if (where == null) return DqlStatement.EMPTY_STATEMENT;
                  return where(
                      "a.[documentLine as dl].(" + where.getDql() + ')',
                      where.getParameterValues());
                })
            .ifNotNullOtherwiseEmpty(
                pm.groupDqlStatementProperty(),
                groupDqlStatement -> {
                  DqlClause groupBy = groupDqlStatement.getGroupBy();
                  if (groupBy == null) return DqlStatement.EMPTY_STATEMENT;
                  String dqlGroupBy = "documentLine.(" + groupBy.getDql() + ')';
                  return DqlStatement.parse(
                      "{columns: `"
                          + dqlGroupBy
                          + ",date,count(1)`, groupBy: `"
                          + dqlGroupBy
                          + ",date`}");
                });
    ...
    statisticsBuilder =
        new StatisticsBuilder(
                leftGroupVisualMapper,
                rightAttendanceVisualMapper,
                leftGroupVisualMapper.visualResultProperty())
            .start();

    // Setting up the master filter for the content displayed in the master view
    masterVisualMapper =
        ReactiveVisualMapper.<DocumentLine>createMasterReactiveChain(this, pm)
            .always(
