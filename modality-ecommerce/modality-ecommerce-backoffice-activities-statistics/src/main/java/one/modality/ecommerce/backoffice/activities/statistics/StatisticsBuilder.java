...
public class StatisticsBuilder {
    ...
    private final ObjectProperty<VisualResult> leftVisualResultProperty =
            new SimpleObjectProperty<VisualResult /*GWT*/>() {
                @Override
                protected void invalidated() {
                    buildFinalVisualResultIfReady();
                }
            };
    private final ObjectProperty<VisualResult> rightVisualResultProperty =
            new SimpleObjectProperty<VisualResult /*GWT*/>() {
                @Override
                protected void invalidated() {
                    buildFinalVisualResultIfReady();
    ...
    public StatisticsBuilder(
            ReactiveVisualMapper<DocumentLine> leftDocumentLineVisualMapper,
            ReactiveVisualMapper<Attendance> rightAttendanceVisualMapper,
            Property<VisualResult> finalVisualResultProperty) {
    ...
    private void buildFinalVisualResultIfReady() {
        ...
        if (settingFinalResult
                || leftResult == lastLeftResult
                || rightResult == lastRightResult
                || rowCount == 0
                || leftColCount == 0
                || rightAttendances.isEmpty()) return;
        lastLeftResult = leftResult;
        lastRightResult = rightResult;
        List<LocalDate> dates = new ArrayList<>();
        rightAttendances.forEach(
                a -> {
        ...
        for (int col = 0; col < rightColCount; col++)
            columns[leftColCount + col] =
                    VisualColumn.create(
                            Dates.format(dates.get(col), "dd/MM"),
                            PrimType.INTEGER,
                            VisualStyle.RIGHT_STYLE); // , new DisplayStyleImpl(32d, "right"));
        ...
        EntityList<DocumentLine> leftDocumentLines =
                leftDocumentLineVisualMapper.getCurrentEntities();
                ...
                a -> {
                    ...
                    for (int row = 0; row < rowCount; row++) {
                        ...
                        for (int col = 0; col < leftColumns.length - 1; col++) {
                            ...
                            if (!Objects.equals(
                                    leftDocumentLine.evaluate(expression),
                                    rightDocumentLine.evaluate(expression))) {
                                match = false;
                                break;
                            }
                        }
                        if (match) {
                            rsb.setValue(
                                    row,
                                    leftColCount + dates.indexOf(date),
                                    a.getFieldValue("count"));
                            break;
                        }
                    }
                });
