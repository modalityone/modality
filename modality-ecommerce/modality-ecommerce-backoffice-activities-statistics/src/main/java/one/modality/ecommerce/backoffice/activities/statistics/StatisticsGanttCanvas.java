 ...
 * @author Bruno Salmon
 */
public final class StatisticsGanttCanvas {
  ...
  private final LocalDateGanttLayout<Attendance> barsLayout =
      new LocalDateGanttLayout<Attendance>()
  ...
  private final BarDrawer attendanceDrawer =
      new BarDrawer() // unique instance to draw all the bars
          // Setting the unchanging properties (remaining changing properties will be set in
          // drawBar())
          .setTextFill(Color.WHITE)
          .setBackgroundFill(Color.rgb(65, 186, 77))
          .setClipText(false) // doesn't need clipping (better perf)
      ;
  private final BarDrawer parentItemDrawer =
  ...
  private final BarDrawer grandparentItemFamilyDrawer =
      new BarDrawer() // unique instance to draw all the room types
          // Setting the unchanging properties (remaining changing properties will be set in
          // drawGrandParentItemFamily())
  ...
  public StatisticsGanttCanvas() {
    ...
    pm.bindTimeWindow(
        barsLayout); // barsLayout will itself be bound to FXGanttTimeWindow (see below)
    ...
    ThemeRegistry.runNowAndOnModeChange(
        () -> {
          ...
          grandparentItemFamilyDrawer.setTextFont(
              TextTheme.getFont(FontDef.font(FontWeight.BOLD, 13)));
        });
  }
  ...
  public Node buildCanvasContainer() {
    ...
    // position.
    VirtualCanvasPane roomsAndBarsPane =
  ...
  private void drawAttendance(Attendance a, Bounds b, GraphicsContext gc) {
    ...
    attendanceDrawer.setMiddleText(String.valueOf(count)).drawBar(b, gc);
  }
  ...
  private void drawGrandParentItemFamily(ItemFamily item, Bounds b, GraphicsContext gc) {
    grandparentItemFamilyDrawer.setBottomText(item.getName()).drawBar(b, gc);
  }
  ...
  public void startLogic(Object mixin) {
    ReactiveEntitiesMapper.<Attendance>createPushReactiveChain(mixin)
        ...
        .always(
            orderBy(
                "documentLine.item.family.ord,documentLine.item,date")) // Order is important for
                                                                        // TimeBarUtil (see comment
                                                                        // on barsLayout)
        // Returning events for the selected organization only (or returning an empty set if no
        // organization is selected)
        .ifNotNullOtherwiseEmpty(
            pm.organizationIdProperty(),
            o -> where("documentLine.document.event.organization=?", o))
