...
final class PersonActivity extends BookingProcessActivity {
    ...
    protected void createViewNodes() {
        ...
        */
        ...
        FlowPane accountTabs =
                new FlowPane(
                        new Button(null, newRadioButton("IDontHaveAnAccount", accountToggleGroup)),
                        new Button(
                                null, newRadioButton("IAlreadyHaveAnAccount", accountToggleGroup)));
        ObservableBooleanValue loggedInProperty = FXLoggedIn.loggedInProperty();
        ObservableBooleanValue notLoggedIn =
                BooleanExpression.booleanExpression(loggedInProperty).not();
        LoginPanel loginPanel = new LoginPanel();
        personalDetailsPanel = new PersonalDetailsPanel(getEvent(), this, pageContainer);
        Node[] tabContents = {
            new VBox(10, personalDetailsPanel.getSectionPanel(), nextButton), loginPanel.getNode()
        };
        ...
        accountToggleGroup
                ...
                .addListener(
                        (observable, oldValue, newValue) -> {
                            Node displayedNode =
                                    tabContents[accountToggleGroup.getToggles().indexOf(newValue)];
                            accountPane.setCenter(displayedNode);
                            if (displayedNode == loginPanel.getNode()) loginPanel.prepareShowing();
                        });
        ...
        FXProperties.runNowAndOnPropertiesChange(
                () -> {
                    if (loggedInProperty.getValue())
                        Platform.runLater(
                                () ->
                                        accountToggleGroup.selectToggle(
                                                accountToggleGroup.getToggles().get(0)));
                },
                loggedInProperty);
        verticalStack
                .getChildren()
    ...
    public void onPause() {
        super.onPause();
        syncModelFromUi();
        // Clearing the computed price cache on leaving this page in case a personal detail
        // affecting the price (such as age) has been changed
        WorkingDocument workingDocument = getEventActiveWorkingDocument();
        if (workingDocument != null) workingDocument.clearComputedPrice();
    }
}
