...
public class ServerDelegatedPaymentProvider implements DelegatedPaymentProvider {
    ...
    public Future<InitiateDelegatedPaymentResult> initiateDelegatedPayment(InitiateDelegatedPaymentArgument argument) {
        Iterator<DelegatedPaymentGatewayProvider> it = getDelegatedPaymentGatewayProviders().iterator();
        if (it.hasNext())
            return it.next().initiateDelegatedPayment(new InitiateDelegatedPaymentGatewayArgument(argument, STRIPE_API_KEY, PAYMENT_SUCCESS_URL, PAYMENT_CANCEL_URL));
        return Future.failedFuture(new IllegalStateException("No delegated payment gateway found!"));
    }

    protected ServiceLoader<DelegatedPaymentGatewayProvider> getDelegatedPaymentGatewayProviders() {
        return ServiceLoader.load(DelegatedPaymentGatewayProvider.class);
    }
