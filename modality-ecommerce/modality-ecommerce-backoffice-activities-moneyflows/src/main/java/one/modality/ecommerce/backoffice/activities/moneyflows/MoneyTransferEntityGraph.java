 ...
 * @author Dan Newman
 */
public class MoneyTransferEntityGraph extends Pane
  ...
  public ObservableList<MoneyAccountPane> moneyAccountPanes() {
    return moneyAccountPanes;
  }

  private final ObservableList<MoneyFlowArrowView> moneyFlowArrowViews =
      FXCollections.observableArrayList();

  public ObservableList<MoneyFlowArrowView> moneyFlowArrowViews() {
    return moneyFlowArrowViews;
  }

  private final ObjectProperty<MoneyAccount> selectedMoneyAccount = new SimpleObjectProperty<>();

  public ObjectProperty<MoneyAccount> selectedMoneyAccount() {
    return selectedMoneyAccount;
  }

  private final ObjectProperty<MoneyFlow> selectedMoneyFlow = new SimpleObjectProperty<>();

  public ObjectProperty<MoneyFlow> selectedMoneyFlow() {
  ...
  public MoneyTransferEntityGraph() {
    ...
    selectedMoneyFlow.addListener(
        (observable, oldValue, newValue) ->
            moneyFlowArrowViews()
                .forEach(
                    arrow -> {
                      arrow.setHighlighted(
                          newValue != null && newValue.equals(arrow.moneyFlowProperty().get()));
                    }));
  }
  ...
  private void updateLayout() {
    ...
    for (Entry<Integer, List<MoneyAccount>> entry : nodesByDistanceFromRoot.entrySet()) {
      ...
      int index = 1;
      List<MoneyAccount> alphabetizedMoneyAccounts =
          entry.getValue().stream()
              .sorted(
                  (account1, account2) ->
                      ensureStringNotNull(account1.getName())
                          .compareToIgnoreCase(ensureStringNotNull(account2.getName())))
        ...
        pane.layoutXProperty()
            .bind(
                FXProperties.compute(
                    widthProperty(),
                    width -> 20 + width.doubleValue() * widthNumerator / widthDenominator));
        pane.layoutYProperty()
            .bind(
                FXProperties.compute(
                    heightProperty(),
                    height -> height.doubleValue() * finalIndex / heightDenominator));
        index++;
      }
    }
  }
  ...
  private void removePreviousMoneyAccountPanes() {
    List<Node> previousMoneyAccountPanes =
        getChildren().stream()
  ...
  class MoneyFlowToArrowMapper
      implements IndividualEntityToObjectMapper<MoneyFlow, MoneyFlowArrowView> {
    ...
    MoneyFlowToArrowMapper(MoneyFlow moneyFlow) {
      ...
      moneyFlowArrowView =
          new MoneyFlowArrowView(
              moneyFlow,
              sourceVertex,
              destVertex,
              MoneyTransferEntityGraph.this,
              selectedMoneyFlow);
      moneyAccountPanes.addListener(
          (ListChangeListener<? super MoneyAccountPane>) e -> updateVertices());
    }
