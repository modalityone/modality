...
public class MoneyFlowsActivity extends OrganizationDependentViewDomainActivity
        ...
        newOperationAction(
            () ->
                new EditMoneyAccountRequest(
                    graph.selectedMoneyAccount().get(), moneyAccountTableContainer)),
        newOperationAction(
            () ->
  ...
  private ActionGroup createMoneyFlowTableContextMenuActionGroup() {
    return newActionGroup(
        newOperationAction(
            () ->
                new EditMoneyFlowRequest(graph.selectedMoneyFlow().get(), moneyFlowTableContainer)),
        newOperationAction(
            () ->
  ...
  private void createAddNewMoneyAccountButton() {
    Button addNewMoneyAccountButton =
        newButton(
            newOperationAction(() -> new AddNewMoneyAccountRequest(getOrganization(), graph)));
        ...
        .bind(
            FXProperties.combine(
                ...
                (nodeHeight, buttonHeight) ->
                    nodeHeight.doubleValue() - buttonHeight.doubleValue() - 10));
  ...
  class MoneyAccountToPaneMapper
    ...
    private ActionGroup createContextMenuActionGroup() {
      return newActionGroup(
          newOperationAction(
              () -> new EditMoneyAccountRequest(graph.selectedMoneyAccount().get(), graph)),
          newOperationAction(
              () ->
                  new DeleteMoneyAccountRequest(
