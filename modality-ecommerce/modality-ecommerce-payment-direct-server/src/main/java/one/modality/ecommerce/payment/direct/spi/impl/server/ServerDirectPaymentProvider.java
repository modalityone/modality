...
public class ServerDirectPaymentProvider implements DirectPaymentProvider {
    ...
    private static List<DirectPaymentGatewayProvider> getProviders() {
        return MultipleServiceProviders.getProviders(
                DirectPaymentGatewayProvider.class,
                () -> ServiceLoader.load(DirectPaymentGatewayProvider.class));
    }
    ...
    public Future<MakeDirectPaymentResult> makeDirectPayment(MakeDirectPaymentArgument argument) {
        ...
        if (providers.isEmpty()) return Future.failedFuture("No direct payment gateway found!");
        return providers
                .get(0) // Temporary
    ...
    public GetDirectPaymentGatewayInfosResult getDirectPaymentGatewayInfosInstant(
            GetDirectPaymentGatewayInfosArgument argument) {
        getProviders();
        return null;
    }
