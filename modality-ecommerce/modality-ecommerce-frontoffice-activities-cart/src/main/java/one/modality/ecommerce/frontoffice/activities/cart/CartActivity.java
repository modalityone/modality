...
import one.modality.ecommerce.client.businessdata.workingdocument.WorkingDocumentsByCartStore;
...
import one.modality.event.frontoffice.operations.startbooking.RouteToStartBookingRequest;
...
final class CartActivity extends CartBasedActivity {
  ...
  private final WritableAction modifyBookingAction =
      new WritableAction(
          newAction(
              "<<Modify",
              "{url: 'images/svg/mono/pen.svg', width: 16, height: 16}",
              this::modifyBooking),
          "*");
  private final WritableAction cancelBookingAction =
      new WritableAction(
          newAction(
              "Cancel",
              "{url: 'images/svg/mono/cancel-red.svg', width: 16, height: 16}",
              this::cancelBooking),
          "*");
  private final WritableAction contactUsAction =
      new WritableAction(
          newAction(
              "ContactUs>>",
              "{url: 'images/svg/mono/mail.svg', width: 16, height: 16}",
              this::contactUs),
          "*");
  // private final Action termsAction                 =
  // ModalityActions.newVisitTermsAndConditionsAction(this::readTerms);
  private final WritableAction showPaymentsAction =
      new WritableAction(newAction("YourPayments", this::showPayments), "*");
  private final Action addAnotherBookingAction =
      newAction(
          "<<AddAnotherBooking",
          "{url: 'images/svg/mono/plus-circle-green.svg', width: 32, height: 32}",
  ...
  private final Action makePaymentAction =
      newAction(
          ...
          this::makePayment);
  private final Action explainStatusAction =
      newAction(
          null,
          "{url: 'images/svg/mono/help-circle-blue.svg', width: 32, height: 32}",
          this::explainStatus);
  ...
  private final Property<VisualSelection> documentVisualSelectionProperty =
  ...
  public Node buildUi() {
    ...
    optionsPanel =
        SectionPanelFactory.createSectionPanelWithHeaderNodes(
            ...
            ActionBinder.getAndBindActionIcon(explainStatusAction));
    ...
    FlexBox bookingButtonBar =
        createFlexButtonBar(modifyBookingAction, cancelBookingAction, contactUsAction);

    optionsPanel.setBottom(LayoutUtil.createPadding(bookingButtonBar));

    bottomButtonBar =
        createFlexButtonBar(addAnotherBookingAction, showPaymentsAction, makePaymentAction);
    ...
    return new BorderPane(
        LayoutUtil.createVerticalScrollPaneWithPadding(
            new VBox(20, bookingsPanel, optionsPanel, paymentsPanel, bottomButtonBar)));
  }

  private FlexBox createFlexButtonBar(Action... actions) {
    // not compiling with GWT return new FlexBox(20, 10, Arrays.map(actions, action ->
    // LayoutUtil.setMaxWidthToInfinite(LayoutUtil.setMinWidthToPref(newButton(action))),
    // Node[]::new));
  ...
  private Button createFlexButton(Action action) {
    return LayoutUtil.setMaxWidthToInfinite(
        LayoutUtil.setMinWidthToPref(
            action == makePaymentAction
                ? newGreenButton(action)
                : action == addAnotherBookingAction
                    ? newTransparentButton(action)
                    : newButton(action)));
  }
  ...
  protected void startLogic() {
    ...
    documentVisualSelectionProperty.addListener(
        (observable, oldValue, selection) -> {
                ...
                .onComplete(
                    ar ->
                        UiScheduler.runInUiThread(
                            () -> {
                              setSelectedWorkingDocument(
                                  Collections.get(getCartWorkingDocuments(), selectedRow));
    ...
    UiScheduler.runInUiThread(
        () -> {
          ...
          if (selectedIndex == -1 && eventAggregate() != null)
            selectedIndex =
                indexOfWorkingDocument(
                    ActiveWorkingDocumentsByEventStore.getEventActiveWorkingDocument(getEvent()));
          documentVisualSelectionProperty.setValue(
              VisualSelection.createSingleRowSelection(Math.max(0, selectedIndex)));
  ...
  protected void unloadCart() {
    if (selectedWorkingDocument != null)
      WorkingDocumentsByCartStore.unloadCartWorkingDocuments(
          selectedWorkingDocument.getDocument().getCart());
    super.unloadCart();
  }
  ...
  protected void onCartLoaded() {
    ...
    if (event != null) {
      ...
      displayCartPayments();
      if (selectedWorkingDocument == null) autoSelectWorkingDocument();
    }
  }
    ...
    displayEntities(
        ...
        "["
            ...
            */
            ...
            + "]",
        "Document",
        documentVisualResultProperty);
  }

  private void displayCartPayments() {
    displayEntities(
        cartAggregate().getCartPayments(),
        "["
  ...
  private void displayEntities(
    ...
    visualResultProperty.setValue(
        EntitiesToVisualResultMapper.mapEntitiesToVisualResult(
            entities, columnsDefinition, getDataSourceModel().getDomainModel(), classId));
  }
  ...
  private void displayBookingOptions() {
    if (bookingOptionsPanel != null && selectedWorkingDocument != null) {
      ...
      bookingLabel.setText(
          selectedDocument.getFullName()
              + " - "
              + I18n.getI18nText("Status:")
              + " "
              + I18n.getI18nText(getDocumentStatus(selectedDocument)));
  ...
  private int indexOfWorkingDocument(WorkingDocument workingDocument) {
    if (workingDocument == null) return -1;
    return Collections.indexOf(
        getCartWorkingDocuments(),
        wd -> Entities.sameId(wd.getDocument(), workingDocument.getDocument()));
  }
  ...
  private void disableBookingOptionsButtons(boolean disable) {
    ...
    contactUsAction.setDisabled(
        disable
            || selectedWorkingDocument == null
            || selectedWorkingDocument.getDocument().getEmail() == null);
  }
  ...
  private void updatePaymentsVisibility() {
    if (paymentsPanel != null) {
      ...
      } else showPaymentsAction.setVisible(!paymentsPanel.isVisible());
    }
  }

  private String getDocumentStatus(
      Document
          document) { // TODO: return a structure instead with also background and message to
                      // display in the booking panel (like in javascript version)
    if (document == null) return null;
    if (document.isCancelled()) return "CancelledStatus";
    if (!document.isConfirmed()) {
      if (documentNeedsDeposit(document) && !hasPendingPayment(document)) return "IncompleteStatus";
      return "InProgressStatus";
    }
    if (documentHasBalance(document)) return "ConfirmedStatus";
  ...
  private boolean hasPendingPayment(Document document) {
    return Collections.anyMatch(
        cartAggregate().getCartPayments(), mt -> mt.getDocument() == document && mt.isPending());
  }
