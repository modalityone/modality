...
final class CartActivity extends CartBasedActivity {
    ...
    private final Property<VisualResult> documentVisualResultProperty =
    ...
    private FlexBox createFlexButtonBar(Action... actions) {
        ...
        Arrays.forEach(
                actions, action -> flexButtonBar.getChildren().add(createFlexButton(action)));
                    ...
                    if (selectedIndex == -1 && eventAggregate() != null)
                        selectedIndex =
                                indexOfWorkingDocument(
                                        ActiveWorkingDocumentsByEventStore
                                                .getEventActiveWorkingDocument(getEvent()));
    ...
    private void displayEntities(
        ...
        visualResultProperty.setValue(
                EntitiesToVisualResultMapper.mapEntitiesToVisualResult(
                        entities,
                        columnsDefinition,
                        getDataSourceModel().getDomainModel(),
                        classId));
    }
    ...
    private boolean hasPendingPayment(Document document) {
        return Collections.anyMatch(
                cartAggregate().getCartPayments(),
                mt -> mt.getDocument() == document && mt.isPending());
    }
                        ...
                        .addButtons(
                                ...
                                dialogCallback -> {
                                    ...
                                    Document selectedDocument =
                                            selectedWorkingDocument.getDocument();
                                    UpdateStore updateStore =
                                            UpdateStore.createAbove(selectedDocument.getStore());
                                    Document updatedDocument =
                                            updateStore.updateEntity(selectedDocument);
