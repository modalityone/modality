...
public class ServerPolicyServiceProvider implements PolicyServiceProvider {
    ...
    private final static String SCHEDULED_ITEMS_QUERY_BASE = "select site.name,item.(name,label,code,family.(code,name,label),ord),date,startTime,timeline.startTime,cancelled from ScheduledItem";
    private final static String RATES_QUERY_BASE = "select site,item,price,perDay,perPerson,facilityFee_price,startDate,endDate,onDate,offDate,minDeposit,cutoffDate,minDeposit2,age1_max,age1_price,age1_discount,age2_max,age2_price,age2_discount,resident_price,resident_discount,resident2_price,resident2_discount from Rate";
    private final static String BOOKABLE_PERIODS_QUERY_BASE = "select startScheduledItem,endScheduledItem,name,label from BookablePeriod";
    private final static String ITEM_POLICIES_QUERY_BASE = "select scope.(organization,site,eventType,event),minDay from ItemPolicy";
        ...
        return QueryService.executeQueryBatch(
                new Batch<>(new QueryArgument[]{
                    ...
                    DqlQueries.newQueryArgumentForDefaultDataSource(
                        SCHEDULED_ITEMS_QUERY_BASE + " where event = (select coalesce(repeatedEvent, id) from Event where id=$1) and bookableScheduledItem=id " +
