...
public class OptionsActivity extends BookingProcessActivity {
  ...
  protected void startLogic() {
    boolean forceRefresh =
        true; // getEventOptions() == null; // forcing refresh in case the working document has
              // changed (ex: going back from the personal details after having changed the age)
    onEventFeesGroups()
            ...
            feesGroups -> {
              ...
              if (workingDocument == null
                  || selectedOptionsPreselection != null
                      && selectedOptionsPreselection.getWorkingDocument() == workingDocument) {
                ...
                // Ensuring the working document is a duplication of the preselection one to not
                // alter the original one
  ...
  private OptionsPreselection findNoAccommodationOptionsPreselection(FeesGroup[] feesGroups) {
    for (FeesGroup feesGroup : feesGroups) {
      OptionsPreselection noAccommodationPreselection =
          Arrays.findFirst(feesGroup.getOptionsPreselections(), op -> !op.hasAccommodation());
