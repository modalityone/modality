...
final class StartBookingActivity extends BookingProcessActivity {
  ...
  protected Node styleUi(Node uiNode) {
    ...
    onEvent()
        .onComplete(
            ar -> {
              onEventOptions(); // Anticipating event options loading now (required for options and
                                // fees pages)
                        ...
                        eventImageView
                            ...
                            .bind(
                                FXProperties.combine(
                                    eventImageViewContainer.widthProperty(),
                                    image.widthProperty(),
                                    (w1, w2) -> Math.min(w1.doubleValue(), w2.doubleValue())));
                      if (image == null || !image.isBackgroundLoading()) runFadeInAnimation();
                      else image.heightProperty().addListener(observable -> runFadeInAnimation());
                    }
                  });
            });
