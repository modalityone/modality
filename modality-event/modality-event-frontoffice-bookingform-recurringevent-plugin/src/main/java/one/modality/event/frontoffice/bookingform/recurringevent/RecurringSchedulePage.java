 ...
 * @author Bruno Salmon
 */
final class RecurringSchedulePage implements BookingFormPage {
    ...
    public RecurringSchedulePage(RecurringEventBookingForm bookingForm) {
        ...
        checkoutButton.setOnAction((e -> {
            bookingForm.syncWorkingBookingFromScheduledItemsSelector();
    ...
    public void setWorkingBookingProperties(WorkingBookingProperties workingBookingProperties) {
        ...
        BoxScheduledItemsSelector boxScheduledItemsSelector = bookingForm.getScheduledItemBoxesSelector();
        // If the date in not selectable for any reason listed above, we do nothing when we click on the date
        boxScheduledItemsSelector.setOnDateClicked(localDate -> {
            if (nonSelectableDate.contains(localDate)) {
                return;
            }
            boxScheduledItemsSelector.processClickedDate(localDate);
            // Note: changes made on scheduledItemBoxesSelector are not automatically reflected on the working booking at
        ...
        boxScheduledItemsSelector.setUnselectedDateCssGetter((localDate -> {
            ...
            return boxScheduledItemsSelector.getUnselectedDateCssClass();
        }));

        /* UX Designer asked to remove this
        scheduledItemBoxesSelector.setComputeNodeForExistingBookedDateFunction((localDate -> {
            if(alreadyBookedDate.contains(localDate)) {
        ...
        bookingForm.syncScheduledItemsSelectorFromWorkingBooking();
        ...
        selectAllClassesHyperlink.setOnAction((e -> boxScheduledItemsSelector.addClickedDates(allSelectableDates)));
    ...
    public ObservableBooleanValue validProperty() { // TODO not create a new instance each time
        BookEventActivity activity = bookingForm.getActivity();
        WorkingBookingProperties workingBookingProperties = activity.getWorkingBookingProperties();
        ObservableList<LocalDate> selectedDates = bookingForm.getScheduledItemBoxesSelector().getSelectedDates();
