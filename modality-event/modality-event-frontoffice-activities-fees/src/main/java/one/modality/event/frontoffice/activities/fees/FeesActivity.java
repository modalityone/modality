...
final class FeesActivity extends BookingProcessActivity {
  ...
  protected void createViewNodes() {
    ...
    verticalStack
        .getChildren()
        .setAll(feesGroupsCollator, LayoutUtil.setMaxWidthToInfinite(backButton));
    ...
    FXProperties.consume(
        FXProperties.filter(
            FXProperties.combine(
                I18n.dictionaryProperty(), activeProperty(), Pair::new), // combine function
            pair ->
                pair.get2()), // filter function (GWT doesn't compile method reference in this case)
  ...
  private void displayFeesGroupsAndRefreshAvailabilities(FeesGroup[] feesGroups) {
    ...
    onEventAvailabilities()
        .onComplete(
            ar -> {
              if (ar.succeeded()) displayFeesGroups();
            });
    if (getEventAvailabilities() == null) displayFeesGroups();
  }

  private void displayFeesGroups() {
    if (getEvent() == null
        || feesGroups
            == null) // This can happen when reacting to active property while the event has just
  ...
  private void displayFeesGroupsNow() {
    ...
    VisualResultBuilder rsb =
        VisualResultBuilder.create(
            n,
            new VisualColumn[] {
              VisualColumn.create(
                  (value, context) ->
                      renderFeesGroupHeader((Pair<ReadOnlyJsonObject, String>) value)),
    ...
    ColumnWidthCumulator[] cumulators = {
      new ColumnWidthCumulator(), new ColumnWidthCumulator(), new ColumnWidthCumulator()
    };
    for (int i = 0; i < n; i++) {
      FeesGroup feesGroup = feesGroups[i];
      rsb.setValue(i, 0, new Pair<>(jsonImage, feesGroup.getDisplayName()));
      rsb.setValue(
          i, 1, generateFeesGroupVisualResult(feesGroup, this::onBookButtonPressed, cumulators));
  ...
  private VisualResult generateFeesGroupVisualResult(
      FeesGroup feesGroup,
      Consumer<OptionsPreselection> bookHandler,
      ColumnWidthCumulator[] cumulators) {
    ModalityButtonFactoryMixin buttonFactory = this;
    EventAggregate eventAggregate = this;
    boolean showBadges =
        Objects.areEquals(
            eventAggregate.getEvent().getOrganizationId().getPrimaryKey(),
            2); // For now only showing badges on KMCF courses
    ...
    VisualResultBuilder rsb =
        VisualResultBuilder.create(
            optionsCount,
            VisualColumnBuilder.create(
                    I18n.getI18nText(
                        singleOption
                            ? (feesGroup.isFestival() ? "Festival" : "Course")
                            : "Accommodation"),
                    PrimType.STRING)
                .setCumulator(cumulators[0])
                .build(),
            VisualColumnBuilder.create(I18n.getI18nText("Fee"), PrimType.INTEGER)
                .setStyle(VisualStyle.CENTER_STYLE)
                .setCumulator(cumulators[1])
                .build(),
            VisualColumnBuilder.create(I18n.getI18nText("Availability"))
                .setStyle(VisualStyle.CENTER_STYLE)
                .setCumulator(cumulators[2])
                .setValueRenderer(
                    (p, context) -> {
                      Pair<Object, OptionsPreselection> pair =
                          (Pair<Object, OptionsPreselection>) p;
                      if (pair == null || !eventAggregate.areEventAvailabilitiesLoaded())
                        return new ImageView(
                            ImageStore.getOrCreateImage(ModalityIcons.spinnerIcon16Url, 16, 16));
                      ...
                      if (availability == null
                          && optionsPreselection
                              .hasAccommodationExcludingSharing()) // ... if it's an accommodation
                                                                   // option (but not just sharing)
                      availability =
                            0; // we show it as sold out - otherwise (if it's a sharing option or no
                               // accommodation) we show it as available
                      boolean soldout =
                          availability != null && Numbers.doubleValue(availability) <= 0
                              || // Showing sold out if the availability is zero
                              optionsPreselection.isForceSoldout()
                              || // or if the option has been forced as sold out in the back-office
                              feesGroup
                                  .isForceSoldout(); // or if the whole FeesGroup has been forced as
                                                     // sold out
                      ...
                      if (availability == null || !showBadges) return button;
                      HBox hBox =
                          (HBox)
                              NodeCollatorRegistry.hBoxCollator()
                                  .collateNodes(
                                      createBadge(
                                          TextRenderer.SINGLETON.renderValue(
                                              availability,
                                              ValueRenderingContext.DEFAULT_READONLY_CONTEXT)),
                                      button);
    ...
    for (OptionsPreselection optionsPreselection : optionsPreselections) {
      rsb.setValue(
          ...
          0,
          singleOption
              ? /* Showing course name instead of 'NoAccommodation' when single line */ Labels
                  .instantTranslateLabel(
                      Objects.coalesce(
                          feesGroup.getLabel(), Labels.bestLabelOrName(feesGroup.getEvent())))
              : /* Otherwise showing accommodation type */ optionsPreselection.getDisplayName());
      rsb.setValue(rowIndex, 1, optionsPreselection.getDisplayPrice());
      rsb.setValue(
          rowIndex++,
          2,
          new Pair<>(
              optionsPreselection.getDisplayAvailability(eventAggregate), optionsPreselection));
    }
    return rsb.build();
  }
  ...
  private Node renderFeesGroupHeader(Pair<ReadOnlyJsonObject, String> pair) {
    ...
    RadioButton noDiscountRadio =
        hasDiscountRates ? I18nControls.setI18nProperties(new RadioButton(), "NoDiscount") : null;
    RadioButton unemployedRadio =
        hasUnemployedRate
            ? I18nControls.setI18nProperties(new RadioButton(), "UnemployedDiscount")
            : null;
    RadioButton facilityFeeRadio =
        hasFacilityFeeRate
            ? I18nControls.setI18nProperties(new RadioButton(), "FacilityFeeDiscount")
            : null;
    ...
    if (unemployedRadio != null) {
      ...
      unemployedRadio
          .selectedProperty()
          .addListener(
              (observable, oldValue, unemployed) -> {
                person.setUnemployed(unemployed);
                if (unemployed) person.setFacilityFee(false);
                displayFeesGroups();
              });
    }
    if (facilityFeeRadio != null) {
      ...
      facilityFeeRadio
          .selectedProperty()
          .addListener(
              (observable, oldValue, facilityFee) -> {
                person.setFacilityFee(facilityFee);
                if (facilityFee) person.setUnemployed(false);
                displayFeesGroups();
              });
    }
    if (noDiscountRadio != null) {
      noDiscountRadio.setSelected(
          Booleans.isNotTrue(person.isUnemployed()) && Booleans.isNotTrue(person.isFacilityFee()));
      noDiscountRadio
          .selectedProperty()
          .addListener(
              (observable, oldValue, noDiscount) -> {
                if (noDiscount) {
  ...
  private Node renderFeesGroupBody(VisualResult rs) {
    VisualGrid visualGrid =
        new SkinnedVisualGrid(rs); // LayoutUtil.setMinMaxHeightToPref(new DataGrid(rs));
