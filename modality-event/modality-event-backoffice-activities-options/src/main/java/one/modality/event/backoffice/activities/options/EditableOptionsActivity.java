...
final class EditableOptionsActivity extends OptionsActivity {
  ...
  protected void createViewNodes() {
    ...
    FXProperties.runOnPropertiesChange(
        () -> ((EditableBookingCalendar) bookingCalendar).setEditMode(editModeProperty.getValue()),
        editModeProperty);
    ...
    HBox hbox =
        new HBox(
            20, addOptionButton, createHGrowable(), editModeCheckBox, createHGrowable(), priceText);
              ...
              .onSuccess(
                  resultBatch -> {
                    ...
                    getEventActiveWorkingDocument()
                        ...
                        .removeIf(line -> getTopParentOption(line.getOption()) == option);
                    getEventActiveOptionsPreselection()
                        .getOptionPreselections()
                        .removeIf(
                            optionPreselection ->
                                getTopParentOption(optionPreselection.getOption()) == option);
                    clearEventOptions();
                    startLogic();
                  });
        },
    ...
    if (addOptionDialogPane == null) {
      ...
      addOptionDialogVisualMapper =
          ReactiveVisualMapper.<Option>createPushReactiveChain(this)
              ...
              .always(
                  eventIdProperty(),
                  e -> where("event.organization=?", getEvent().getOrganization()))
              .setEntityColumns(
                  "["
                        ...
                        ar2 -> {
                          if (ar2.succeeded()) {
                            for (FeesGroup feesGroup : ar2.result()) {
                              for (OptionsPreselection optionsPreselection :
                                  feesGroup.getOptionsPreselections()) {
                                if (optionsPreselection.getLabel()
                                    == selectedOptionsPreselection.getLabel()) {
