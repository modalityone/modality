...
public final class EditableBookingCalendar extends BookingCalendar {
      ...
      if (option != null) {
        DayTimeRangeEditor.showDayTimeRangeEditorDialog(
            ...
            (newDayTimeRange, dialogCallback) -> {
              ...
              UpdateStore store =
                  UpdateStore.create(workingDocument.getEventAggregate().getDataSourceModel());
  ...
  private static Option getCalendarTimelineOption(CalendarTimeline calendarTimeline) {
    ...
    if (source instanceof WorkingDocumentLine) return ((WorkingDocumentLine) source).getOption();
    return null;
  }
}
