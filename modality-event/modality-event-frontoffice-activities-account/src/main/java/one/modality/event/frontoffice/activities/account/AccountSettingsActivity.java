...
public class AccountSettingsActivity extends ViewDomainActivityBase {
    private static Node createRow(String label, Node field, Node operator) {
        return Utility.createSplitRow(
                Utility.createField(label, field),
                operator,
                80,
                10
        );
    ...
    public Node buildUi() {
        ...
        container.getChildren().addAll(
                ...
                createRow(
                        "Phone number",
                        Utility.createHList(10, 10,ukIcon, new Text("0044 745 4125 41")),
                        new Button("Change")
                ),
                createRow("Password", new Text("......."), new Button("Change")),
                createRow("Security",
                        Utility.createVList(0, 0,
                                new Text("2-step verification"),
                                Utility.createSplitRow(new Text("Request code on"), new Text("SMS"), 50, 0)),
                        new CheckBox()
                ),
                Utility.createVList(2, 10,
                        new Text("Notifications"),
                        Utility.createSplitRow(new Text("In-push notifications"), new CheckBox(), 80, 0),
                        Utility.createSplitRow(new Text("In-app notifications"), new CheckBox(), 80, 0),
                        Utility.createSplitRow(new Text("Newsletter emails"), new CheckBox(), 80, 0),
                        Utility.createSplitRow(new Text("Special events emails"), new CheckBox(), 80, 0)
                ),
                deleteButton
        );

        return Utility.bindButtonWithPopup(deleteButton, container, deleteContent, 200);
    }
}
