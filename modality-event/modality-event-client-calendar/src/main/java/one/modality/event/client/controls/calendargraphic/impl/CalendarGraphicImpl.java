...
public final class CalendarGraphicImpl implements CalendarGraphic {
  ...
  private void createRootNode() {
    ...
    rootNode
        .widthProperty()
        .addListener((observable, oldValue, newWidth) -> updateTotalWidth(newWidth.doubleValue()));
  ...
  private void createOrUpdateRootNodeCalendar() {
    ...
    if (create) {
      ...
      if (rotate != null) {
        ...
        timeline
            .getKeyFrames()
            .setAll(new KeyFrame(Duration.seconds(5), new KeyValue(rotate.angleProperty(), 360d)));
  ...
  private void addTimelineNodes(
    ...
    Collections.forEach(
        ...
        epochDay ->
            addBlockNodes(epochDay, timeline.getDayTimeRange(), timeline, bodyNodes, index));
  }
