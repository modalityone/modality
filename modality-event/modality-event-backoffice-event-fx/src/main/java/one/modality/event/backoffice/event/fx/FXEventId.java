...
public final class FXEventId {
    ...
    private static final ObjectProperty<EntityId> eventIdProperty =
            new SimpleObjectProperty<>() {
                @Override
                protected void invalidated() {
                    // Getting the eventId that just changed
                    EntityId eventId = getEventId();
                    // Storing this new value (more precisely the primary key) in the session, and
                    // save it
                    ...
                    // Synchronizing FXOrganization to match that new organization id
                    // (FXOrganizationId => FXOrganization)
                    if (!Objects.equals(
                            eventId, FXEvent.getEventId())) { // Sync only if ids differ.
                        // If the new organization id is null, we set the FXOrganization to null
                        if (Entities.getPrimaryKey(eventId) == null) FXEvent.setEvent(null);
                        else {
                            ...
                            // If yes, there is no need to request the server, we use directly that
                            // instance
                            if (event != null) FXEvent.setEvent(event);
                            else // Otherwise, we request the server to load that event from that id
                            eventStore
                                        .<Event>executeQuery(
                                                "select icon,name,startDate,endDate from Event where id=?",
                                                eventId)
    ...
    static {
        ...
        FXProperties.runNowAndOnPropertiesChange(
                () -> {
                    Session session = FXSession.getSession();
                    Object primaryKey =
                            session == null ? null : session.get(SESSION_FX_EVENT_ID_KEY);
    ...
    public static void setEventId(EntityId eventId) {
        if (!Objects.equals(eventId, getEventId())) eventIdProperty.set(eventId);
    }
}
