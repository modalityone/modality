  ...
  private static final ObjectProperty<EntityId> eventIdProperty =
      new SimpleObjectProperty<>() {
        ...
        protected void invalidated() {
          ...
          // Synchronizing FXOrganization to match that new organization id (FXOrganizationId =>
          // FXOrganization)
          if (!Objects.equals(eventId, FXEvent.getEventId())) { // Sync only if ids differ.
            ...
            if (Entities.getPrimaryKey(eventId) == null) FXEvent.setEvent(null);
              ...
              eventStore
                    .<Event>executeQuery(
                        "select icon,name,startDate,endDate from Event where id=?", eventId)
                    .onFailure(System.out::println)
                    .onSuccess(
                        list -> // on successfully receiving the list (should be a singleton list)
                        FXEvent.setEvent(
                                list.isEmpty()
                                    ? null
                                    : list.get(0))); // we finally set FXOrganization
            }
          }
        }
      };
