...
public class BookingCalendar {
  ...
  protected void onCalendarClick(CalendarClickEvent event) {
    if (amendable) {
      ...
      long clickedCellMinuteStart =
          cell.getEpochDay() * 24 * 60 + cell.getDayTimeMinuteInterval().getIncludedStart();
  ...
  private TimeInterval computeNewRequestedDocumentInterval(
    ...
    if (workingIncludedStart >= workingExcludedEnd) { // this indicates the document is empty
      // In this case, we select the clicked cell (the cell behaves like an empty checkbox that the
      // user ticks)
      ...
      // If that clicked cell was already the arrival cell, we deselect it (the cell behaves like a
      // checked checkbox that the user unticks)
      ...
      // If that clicked cell was already the departure cell, we deselect it (the cell behaves like
      // a checked checkbox that the user unticks)
  ...
  private void updateCalendarWorkingDocumentInterval(TimeInterval newRequestedDocumentInterval) {
    ...
    WorkingDocument newWorkingDocument =
        WorkingDocumentMerger.mergeWorkingDocuments(
            workingDocument, newCalendarWorkingDocument, newWorkingDocumentDateTimeRange);
