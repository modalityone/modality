package one.modality.event.client.controls.bookingcalendar;
...
import one.modality.ecommerce.client.businessdata.workingdocument.WorkingDocumentCalendarExtractor;
import one.modality.ecommerce.client.businessdata.workingdocument.WorkingDocumentLine;
import one.modality.ecommerce.client.businessdata.workingdocument.WorkingDocumentMerger;
...
public class BookingCalendar {
    ...
    public void createOrUpdateCalendarGraphicFromOptionsPreselection(
            OptionsPreselection optionsPreselection) {
        createOrUpdateCalendarGraphicFromWorkingDocument(
                optionsPreselection.getWorkingDocument(), false);
    }

    public void createOrUpdateCalendarGraphicFromWorkingDocument(
            WorkingDocument workingDocument, boolean forceRefresh) {
        if (this.workingDocument != workingDocument || forceRefresh) {
            ...
            UiScheduler.runInUiThread(
                    () -> {
                        ...
                        } else calendarGraphic.setCalendar(calendar);
    ...
    private Calendar createCalendarFromWorkingDocument() {
        return WorkingDocumentCalendarExtractor.extractCalendar(
                workingDocument, createNewMaxDateTimeRangeWorkingDocument());
    }

    protected WorkingDocument createNewMaxDateTimeRangeWorkingDocument() {
        return createNewDateTimeRangeWorkingDocument(
                workingDocument.getEventAggregate().getEvent().computeMaxDateTimeRange(), true);
    }

    protected WorkingDocument createNewDateTimeRangeWorkingDocument(
            DateTimeRange workingDocumentDateTimeRange, boolean applyBusinessRules) {
        OptionsPreselection selectedOptionsPreselection =
                ActiveOptionsPreselectionsByEventStore.getActiveOptionsPreselection(
                        workingDocument.getEventAggregate());
        ...
        WorkingDocument newWorkingDocument =
                selectedOptionsPreselection.createNewWorkingDocument(workingDocumentDateTimeRange);
        if (applyBusinessRules) newWorkingDocument.applyBusinessRules();
        return newWorkingDocument;
    }

    private OptionsPreselection findWorkingDocumentOptionsPreselection() {
        for (FeesGroup feesGroup :
                FeesGroupsByEventStore.getEventFeesGroups(workingDocument.getEventAggregate())) {
    ...
    private boolean workingDocumentMatchesOptionsPreselection(
            OptionsPreselection optionsPreselection) {
        return sameLine(
                workingDocument.getAccommodationLine(), optionsPreselection.getAccommodationLine());
    }
    ...
    protected void onCalendarClick(CalendarClickEvent event) {
        if (amendable) {
            ...
            long clickedCellMinuteStart =
                    cell.getEpochDay() * 24 * 60
                            + cell.getDayTimeMinuteInterval().getIncludedStart();
            TimeInterval workingDocumentInterval =
                    workingDocument
                            .getDateTimeRange()
                            .getInterval()
                            .changeTimeUnit(TimeUnit.MINUTES);
            long fromArrival =
                    Math.abs(workingDocumentInterval.getIncludedStart() - clickedCellMinuteStart);
            long fromDeparture =
                    Math.abs(workingDocumentInterval.getExcludedEnd() - clickedCellMinuteStart);
    ...
    private void updateArrivalOrDepartureDateTime(CalendarClickEvent clickEvent, boolean arrival) {
        updateCalendarWorkingDocumentInterval(
                computeNewRequestedDocumentInterval(clickEvent, arrival));
        if (onAttendanceChangedRunnable != null) onAttendanceChangedRunnable.run();
        else createOrUpdateCalendarGraphicFromWorkingDocument(workingDocument, true);
    }

    private TimeInterval computeNewRequestedDocumentInterval(
            CalendarClickEvent clickEvent, boolean arrival) {
        ...
        long clickedIncludedStart =
                clickedDayMinute + clickedDayTimeMinuteInterval.getIncludedStart();
        long clickedExcludedEnd = clickedDayMinute + clickedDayTimeMinuteInterval.getExcludedEnd();
        TimeInterval currentWorkingDocumentInterval =
                workingDocument.getDateTimeRange().getInterval().changeTimeUnit(TimeUnit.MINUTES);
        ...
        if (workingIncludedStart >= workingExcludedEnd) { // this indicates the document is empty
            // In this case, we select the clicked cell (the cell behaves like an empty checkbox
            // that the user ticks)
            ...
            // If that clicked cell was already the arrival cell, we deselect it (the cell behaves
            // like a checked checkbox that the user unticks)
            if (workingIncludedStart >= clickedIncludedStart
                    && workingIncludedStart < clickedExcludedEnd)
                clickedIncludedStart = clickedExcludedEnd; // This will unselect the clicked cell
        } else { // The user specified the departure cell
            // If that clicked cell was already the departure cell, we deselect it (the cell behaves
            // like a checked checkbox that the user unticks)
            if (workingExcludedEnd > clickedIncludedStart
                    && workingExcludedEnd <= clickedExcludedEnd)
                clickedExcludedEnd = clickedIncludedStart; // This will unselect the clicked cell
        }
        return arrival
                ? new TimeInterval(clickedIncludedStart, workingExcludedEnd, TimeUnit.MINUTES)
    ...
    private void updateCalendarWorkingDocumentInterval(TimeInterval newRequestedDocumentInterval) {
        ...
        DateTimeRange newWorkingDocumentDateTimeRange =
                eventAggregate
                        .getEvent()
                        .computeMaxDateTimeRange()
                        .intersect(newRequestedDocumentInterval.toSeries());
        // Logger.log("newWorkingDocumentDateTimeRange: " +
        // newWorkingDocumentDateTimeRange.getText());
        WorkingDocument newCalendarWorkingDocument =
                createNewDateTimeRangeWorkingDocument(newWorkingDocumentDateTimeRange, false);
        // Logger.log("newCalendarWorkingDocument: " +
        // newCalendarWorkingDocument.getDateTimeRange().getText());
        WorkingDocument newWorkingDocument =
                WorkingDocumentMerger.mergeWorkingDocuments(
                        workingDocument,
                        newCalendarWorkingDocument,
                        newWorkingDocumentDateTimeRange);
    ...
    private void displayWorkingTotalPrice() {
        ...
        formattedBookingPrice.setValue(
                EventPriceFormatter.formatWithCurrency(
                        documentPrice, workingDocument.getEventAggregate().getEvent()));
    }
}
