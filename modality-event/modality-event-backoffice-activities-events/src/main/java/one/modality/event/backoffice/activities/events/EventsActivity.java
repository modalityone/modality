...
final class EventsActivity extends ViewDomainActivityBase
    ...
    protected void updateModelFromContextParameters() {
        ...
        if (organizationId != null) pm.setOrganizationId(organizationId);
        else pm.organizationIdProperty().bind(FXOrganizationId.organizationIdProperty());
    }
        ...
        ReactiveVisualMapper.createPushReactiveChain(this)
                ...
                .setEntityColumns(
                        "["
                                ...
                                // "{label: 'Event', expression: 'icon, name + ` ~ ` +
                                // dateIntervalFormat(startDate,endDate) + ` (` + bookingsCount +
                                // `)`'}" +
                ...
                .setSelectedEntityHandler(
                        event -> new RouteToBookingsRequest(event, getHistory()).execute())
                .start();
    }
}
