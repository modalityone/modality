...
public class DatesPicker
{
    ...
    private final ObjectProperty<YearMonth> monthProperty = new SimpleObjectProperty<>(YearMonth.now()) {
        @Override
    ...
    public void focusOnMonth(YearMonth newMonth)
    {
        ...
        if(arrowPreviousMonthPath!=null)arrowPreviousMonthPath.setOnMouseClicked(event -> setMonth(newMonth.minusMonths(1)));
        if(arrowNextMonthPath!=null)arrowNextMonthPath.setOnMouseClicked(event -> setMonth(newMonth.plusMonths(1)));
