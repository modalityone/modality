...
public final class EventsGanttCanvas {
    ...
    // The dated Gantt canvas that already displays dates, weeks, months & years (depending on zoom
    // level)
    private final DatedGanttCanvas datedGanttCanvas =
            new DatedGanttCanvas()
                    // Activating user interaction (user can move & zoom in/out the time window) and
                    // date selection
                    .setInteractive(true)
                    .setDateSelectionEnabled(true);

    // The additional layer that will display the events
    private final LocalDateGanttLayout<Event> eventsLayer =
            new LocalDateGanttLayout<Event>()
    ...
    private final BarDrawer eventBarDrawer =
            new BarDrawer()
                    // Setting up eventBarDrawer global properties (properties specific to events
                    // are set in drawEvent())
                    .setRadius(BAR_RADIUS)
                    .setTextAlignment(null); // auto

    public EventsGanttCanvas() {
        // datedGanttCanvas is registered as the referent in FXGanttTimeWindow for pairing other
        // gantt canvas, so their
        // horizontal time axis stay aligned with this datedGanttCanvas time axis even if their
        // horizontal origin differs
        // (like in the accommodation canvas when we show the legend on the left). To allow this
        // pairing, we need to
        ...
        // Binding the presentation model time window with the UI, here the dated Gantt canvas -
        // probably bound itself to global FXGanttTimeWindow by application code through
        // setupFXBindings()
        pm.organizationIdProperty().bind(FXOrganization.organizationProperty());
        pm.bindTimeWindow(datedGanttCanvas);

        // Passing it to the gantt canvas as an additional layer, that will be automatically drawn
        // using the drawEvent method
    ...
    private void startLogic(Object mixin) {
        ReactiveEntitiesMapper.<Event>createPushReactiveChain(mixin)
                .always(
                        "{class: 'Event', alias: 'e', fields: 'name,startDate,endDate', where: 'active'}")
                // Stopping querying the server when then gantt visibility is not set to EVENTS
                .ifNotEquals(
                        FXGanttVisibility.ganttVisibilityProperty(), GanttVisibility.EVENTS, null)
                ...
                .ifNotNullOtherwiseEmpty(
                        pm.organizationIdProperty(), o -> where("e.organization=?", o))
                // Restricting events to those appearing in the time window
                .always(
                        pm.timeWindowStartProperty(),
                        startDate -> where("e.endDate >= ?", startDate))
                .always(pm.timeWindowEndProperty(), endDate -> where("e.startDate <= ?", endDate))
                // Ordering events in a way that the widest in the time window will be first (=> the
                // smallest events will appear at the bottom)
                .always(
                        pm.timeWindowStartProperty(),
                        startDate -> DqlStatement.orderBy("greatest(e.startDate, ?),id", startDate))
