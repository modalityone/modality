...
public final class EventsGanttCanvas {
  ...
  private final DatedGanttCanvas datedGanttCanvas =
      new DatedGanttCanvas()
          // Activating user interaction (user can move & zoom in/out the time window) and date
          // selection
          .setInteractive(true)
  ...
  private final BarDrawer eventBarDrawer =
      new BarDrawer()
          // Setting up eventBarDrawer global properties (properties specific to events are set in
          // drawEvent())
          .setRadius(BAR_RADIUS)
          .setTextAlignment(null); // auto

  public EventsGanttCanvas() {
    // datedGanttCanvas is registered as the referent in FXGanttTimeWindow for pairing other gantt
    // canvas, so their
    ...
    // Passing it to the gantt canvas as an additional layer, that will be automatically drawn using
    // the drawEvent method
