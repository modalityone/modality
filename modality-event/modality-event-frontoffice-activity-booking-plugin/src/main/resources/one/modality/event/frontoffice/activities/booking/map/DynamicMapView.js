...
async function initMap() {
    ...
    if (javaDynamicView && javaDynamicView.onGoogleMapLoaded)
        javaDynamicView.onGoogleMapLoaded();
}
...
function createMarker(lat, lng, title, javaMarker) {
    ...
    marker.addListener('click', x => {
        javaDynamicView.onMarkerClicked(javaMarker);
    });
    return marker;
}

function injectJavaDynamicView(javaInstance) {
    javaDynamicView   = javaInstance;
    if (googleMap)
        javaDynamicView.onGoogleMapLoaded();
}

if (javaDynamicView)
    injectJavaDynamicView(javaDynamicView);
