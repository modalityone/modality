...
public class CloneEventPresentationViewActivity
  ...
  protected void createViewNodes(CloneEventPresentationModel pm) {
    ...
    // Setting max width/height to pref width/height (otherwise the grid pane takes all space with
    // cells in top left corner)
    ...
    // Now that the grid pane doesn't take all space, we center it (if shown in a border pane which
    // is very probable)
