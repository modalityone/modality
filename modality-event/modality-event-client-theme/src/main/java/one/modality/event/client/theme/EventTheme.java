...
public final class EventTheme { // Used for canvas operations only so far but should implement Theme later for Nodes
    ...
    public static Color getEventBackgroundColor(Event event, boolean selected) {
        return  selected ? Color.rgb(0, 150, 214) :
                //FXPaletteMode.isVariedPalette() && LocalDate.now().isAfter(event.getEndDate()) ? Color.GRAY :
                //FXPaletteMode.isVariedPalette() && LocalDate.now().isBefore(event.getStartDate()) ? Color.ORANGE :
