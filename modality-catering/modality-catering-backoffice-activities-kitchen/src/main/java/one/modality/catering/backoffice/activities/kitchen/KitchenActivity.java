...
public class KitchenActivity extends ViewDomainActivityBase
  ...
  public Node buildUi() {
    ...
    */
    ...
    VBox.setMargin(
        daysOfWeekPane, new Insets(0, WebFxKitLauncher.getVerticalScrollbarExtraWidth(), 0, 0));
    ...
    LuminanceTheme.createPrimaryPanelFacet(container)
        .style(); // To show the same background if the scroll pane doesn't cover the whole area

    // Updating the query each time the selected month or organization change (this will make the
    // reactive call sending the query to the server)
  ...
  private void refreshCalendar() {
    UiScheduler.runInUiThread(
        () ->
            daysOfMonthLayout
                .getChildren()
                .setAll(TimeUtil.generateMonthDates(selectedYearMonthProperty.get())));
  }
  ...
  private void rebuildDayPanels() {
    ...
    for (LocalDate date : attendanceCounts.getDates()) {
      AttendanceDayPanel dayPanel =
          new AttendanceDayPanel(attendanceCounts, date, displayedMeals, abbreviationGenerator);
        ...
        .addListener(
            (observable, oldValue, result) -> {
              ...
              for (int row = 0; row < result.getRowCount(); row++) {
                ...
                if (svg != null) attendanceCounts.storeDietaryOptionSvg(dietaryOptionCode, svg);
