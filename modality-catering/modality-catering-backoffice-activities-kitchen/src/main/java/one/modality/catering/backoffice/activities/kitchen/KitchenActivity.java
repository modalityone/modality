...
public class KitchenActivity extends ViewDomainActivityBase
    ...
    private final ObjectProperty<YearMonth> selectedYearMonthProperty =
    ...
    public Node buildUi() {
        ...
        */
        ...
        // Building the box (TimePane with just 1 row) that will show the days of the week
        // (horizontally)
        ...
        TimePane<DayOfWeek, DayOfWeek> daysOfWeekPane =
                new TimePane<>(daysOfWeekLayout, this::createDayOfWeekNode);
        VBox.setMargin(
                daysOfWeekPane,
                new Insets(0, WebFxKitLauncher.getVerticalScrollbarExtraWidth(), 0, 0));

        // Building the box (TimeGridPane with several rows) that will show each day of the month
        // (calendar layout)
        daysOfMonthLayout = new CalendarLayout<>();
        // daysOfMonthLayout.setFillHeight(true);
        TimeGridPane<LocalDate, LocalDate> daysOfMonthPane =
                new TimeGridPane<>(daysOfMonthLayout, this::createDateNode);
        ...
        FXProperties.runOnPropertiesChange(
                this::updateQueryArgument,
                selectedYearMonthProperty,
        ...
        mealsSelectionPane
                ...
                .addListener((ListChangeListener<Item>) c -> rebuildDayPanels());
        ...
        if (selectedYearMonthProperty.get() == null) setSelectedYearMonth(YearMonth.now());
        ...
        UiScheduler.runInUiThread(
                () ->
                        daysOfMonthLayout
                                .getChildren()
                                .setAll(
                                        TimeUtil.generateMonthDates(
                                                selectedYearMonthProperty.get())));
    }
    ...
    */
    ...
    private Node createDateNode(LocalDate date) {
        ...
        if (attendanceDayPanel != null) return attendanceDayPanel;
    ...
    private void rebuildDayPanels() {
        if (attendanceCounts == null) return;
        ...
        for (LocalDate date : attendanceCounts.getDates()) {
            AttendanceDayPanel dayPanel =
                    new AttendanceDayPanel(
                            attendanceCounts, date, displayedMeals, abbreviationGenerator);
        ...
        Platform.runLater(
                () -> {
                    if (dietaryOptionSvgs.isEmpty()) {
                        // String monthString =
                        // MonthSelectionPanel.buildMonthDisplayText(selectedMonth);
                        // String msg = "No meal data for " + monthString + " for " +
                        // organization.getName();
                        String msg = "No meal data";
                        Label noDataLabel = new Label(msg);
                        TextTheme.createPrimaryTextFacet(noDataLabel)
                                .requestedFont(NO_DATA_MSG_FONT)
                                .style();
    ...
    private static final String MEAL_COUNT_SQL =
    ...
    private void updateQueryArgument() {
        ...
        if (selectedYearMonth == null || organizationId == null) return;

        QueryArgument queryArgument =
                new QueryArgumentBuilder()
                        .setStatement(MEAL_COUNT_SQL)
    ...
    protected void startLogic() {
        ...
        reactiveQueryCall
                .resultProperty()
                .addListener(
                        (observable, oldValue, result) -> {
                            attendanceCounts = new AttendanceCounts();
                            attendanceCounts.storeDietaryOptionSvg(
                                    "Total",
                            ...
                            for (int row = 0; row < result.getRowCount(); row++) {
                                ...
                                attendanceCounts.storeDietaryOptionOrder(
                                        dietaryOptionCode, dietaryOptionOrdinal);
                                String svg = result.getValue(row, 6);
                                if (svg != null)
                                    attendanceCounts.storeDietaryOptionSvg(dietaryOptionCode, svg);
                                if (dietaryOptionCode != null
                                        && dietaryOptionName != null
                                        && svg != null) {
                                    String dietaryOptionKeyText =
                                            dietaryOptionName + " (" + dietaryOptionCode + ")";
