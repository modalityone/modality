...
import one.modality.catering.backoffice.operations.entities.allocationrule.AddNewAllocationRuleRequest;
import one.modality.catering.backoffice.operations.entities.allocationrule.DeleteAllocationRuleRequest;
import one.modality.catering.backoffice.operations.entities.allocationrule.EditAllocationRuleRequest;
import one.modality.catering.backoffice.operations.entities.allocationrule.TriggerAllocationRuleRequest;
import one.modality.ecommerce.backoffice.activities.statistics.StatisticsBuilder;

final class DiningAreasActivity extends EventDependentViewDomainActivity
        ...
        () ->
            newActionGroup(
                newSeparatorActionGroup(
                    newOperationAction(
                        () -> new AddNewAllocationRuleRequest(getEvent(), container))),
                newSeparatorActionGroup(
                    newOperationAction(
                        () ->
                            new TriggerAllocationRuleRequest(
                                rulesVisualMapper.getSelectedEntity(), container))),
                newSeparatorActionGroup(
                    newOperationAction(
                        () ->
                            new EditAllocationRuleRequest(
                                rulesVisualMapper.getSelectedEntity(), container)),
                    newOperationAction(
                        () ->
                            new DeleteAllocationRuleRequest(
                                rulesVisualMapper.getSelectedEntity(), container))),
                newSeparatorActionGroup(
                    newOperationAction(
                        () ->
                            new CopySelectionRequest(
                                rulesVisualMapper.getSelectedEntities(),
                                rulesVisualMapper.getEntityColumns())),
                    newOperationAction(
                        () ->
                            new CopyAllRequest(
                                rulesVisualMapper.getCurrentEntities(),
                                rulesVisualMapper.getEntityColumns())))));
    return container;
  }
  ...
  protected void startLogic() {
    ...
    leftSittingVisualMapper =
        ReactiveVisualMapper.<DocumentLine>createReactiveChain(this)
            .always(
    ...
    rightAttendanceVisualMapper =
        ReactiveVisualMapper.<Attendance>createReactiveChain(this)
            .always(
            ...
            // Applying the event condition
            .ifNotNullOtherwiseEmpty(
                pm.eventIdProperty(), eventId -> where("documentLine.document.event=?", eventId));
    ...
    statisticsBuilder =
        new StatisticsBuilder(
                leftSittingVisualMapper,
                rightAttendanceVisualMapper,
                pm.sittingVisualResultProperty())
            .start();
